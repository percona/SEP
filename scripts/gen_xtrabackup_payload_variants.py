...
It stays the complete, human-edited source, and the other selections are generated
beside it.
...
def _load_naming_rule() -> ModuleType:
    ...
    Loaded by path rather than imported: ``app.extensions.apps.mysql_backups.__init__``
...
#: omits the provider must not mention any of them, which is how the generator
...
_BANNER = (
    ...
    "# Canonical source: app/extensions/apps/mysql_backups/xtrabackup_payload\n"
...
def render(source: str, providers: tuple[str, ...]) -> str:
    ...
    never looks like a second canonical source. Blank-line runs are then capped at
    two, so a dropped region leaves no widening gap where it used to be. The cap is
    not scoped to those gaps: it runs over the whole file, so it also closes any
    longer run the canonical already carried.
...
def validate(text: str, omitted: tuple[str, ...], name: str) -> None:
    ...
    would only fail at backup runtime, on a customer host, so the generator refuses
    to write it. Two passes catch that: an unbound-name sweep, which needs
...
def main(argv: list[str] | None = None) -> int:
    ...
    :return: ``0`` when every variant is in sync (or was rewritten); ``1`` when the
        canonical payload carries no ``GEN:UPLOAD-BEGIN`` markers, when ``--check``
        finds drift, or when a variant fails validation.
    """
