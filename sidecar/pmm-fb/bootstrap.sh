...
passwords, the side-car's ENCRYPTION_KEY, and, on an arm64 engine,
SEP_MYSQL_PLATFORM plus SEP_MYSQL_PMM_CLIENT_IMAGE, which decide how the
executor is built. An existing .env is kept and any slot it predates is
appended. Run before the first `docker compose up -d` — every bring-up needs
it now, not just the mysql profile — after rotating a secret, and after
...
seed_missing_slots() {
    ...
    ensure_slot ENCRYPTION_KEY "$(new_encryption_key)"
}

# Fernet wants 32 url-safe base64-encoded bytes, so the hex the passwords use is
# rejected outright and the +/ of standard base64 has to be translated. Seeded
# once and never rotated by this script: the key outlives the process that wrote
# with it, so a fresh one makes everything already encrypted unreadable.
new_encryption_key() {
...
# ENCRYPTION_KEY is a secret too, and the one value here the side-car itself
# reads: it stands in until the embedded provisioning mints one (README.md
# § Caveats). Rotating it orphans everything already encrypted.
