...
# DEBUG=1 traces every expansion, and the minted token reaches the commands
# wrapped below as arguments — as does the whole secrets file when it is read
# back for rewriting.
without_xtrace() {
    local -
    set +o xtrace
...
# Wrapped by without_xtrace: applying the PMM_ADMIN_PASSWORD default is itself
# an assignment, so tracing it prints an operator-supplied password
apply_pmm_defaults() {
...
without_xtrace apply_pmm_defaults
SA_NAME="sep-fb"
...
grafana_api() {
    # Suspended here rather than via without_xtrace at the call sites, because
    # PMM_ADMIN_PASSWORD is expanded in this frame — a wrapper would only see
    # the argument list after the trace line had already printed it
    local -
    set +o xtrace
...
# A function so without_xtrace can wrap it: the assignment and the format check
# below both expand the minted token, and a trace of either puts it in the log
mint_token() {
...
without_xtrace mint_token || exit 1
...
# A function for the same reason as mint_token: both branches expand the token,
# and the read-back holds every other secret in the file
write_token() {
...
without_xtrace write_token || exit 1
