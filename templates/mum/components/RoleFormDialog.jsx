/**
 * RoleFormDialog — shared dialog for creating and editing MongoDB custom roles.
 *
 * Props:
 *   open          {bool}
 *   onClose       {fn}
 *   onSubmit      {async fn({ role, db, privileges, inheritedRoles })} — throws on error
 *   initialValues {null | { role, db, privileges, inheritedRoles }} — null = create mode
 *   rolesData     {array}  — full roles list for inherited-roles selector
 *   title         {string}
 *   submitLabel   {string}
 *   editMode      {bool}   — when true, role name and db are read-only
 */
...
const buildPrivileges = (privileges, effectiveDb) => {
  ...
  const set = (field) => (e) => {
    if (field === "resourceType") {
      ...
      onChange(index, {
        ...
        actions: priv.actions.filter((a) => validActions.includes(a)),
      });
    } else {
  ...
  return (
    <FormControl component="fieldset" fullWidth disabled={disabled}>
      ...
      {value.length > 0 && (
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5, mt: 0.5 }}>
          {value.map((r) => (
            <Chip key={`${r.role}@${r.db}`} label={`${r.role} @ ${r.db}`} size="small"
...
const RoleFormDialog = ({
  ...
  // Re-initialise form every time the dialog opens.
  useEffect(() => {
    if (!open) return;
    if (initialValues) {
