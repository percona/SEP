...
#    type: str
#    label: Log file path
#    description: The path to your ProxySQL log file
...
PROXYSQL_LOG="${LOG_FILE_ARG:-$DEFAULT_PROXYSQL_LOG}"

# Check if the log file exists and is readable
if [ ! -f "$PROXYSQL_LOG" ]; then
    echo "Error: ProxySQL log file not found at '$PROXYSQL_LOG'."
    echo "Please ensure the file exists and the path is correct."
    exit 1
fi

if [ ! -r "$PROXYSQL_LOG" ]; then
    echo "Error: Cannot read ProxySQL log file at '$PROXYSQL_LOG'."
    echo "Please check file permissions for '$PROXYSQL_LOG'."
    exit 1
fi
