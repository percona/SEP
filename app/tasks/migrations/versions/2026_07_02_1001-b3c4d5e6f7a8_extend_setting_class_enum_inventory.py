...
"""extend setting_class enum with INVENTORY_SETTINGS

Revision ID: b3c4d5e6f7a8
Revises: 8657d05d27da
Create Date: 2026-07-02 10:01:00.000000
...
_NEW_MEMBERS = (*_EXISTING_MEMBERS, "INVENTORY_SETTINGS")
...
def downgrade() -> None:
    """Remove ``INVENTORY_SETTINGS`` from the setting_class constraint."""
    bind = op.get_bind()
    acquire_pg_advisory_xact_lock(bind, SETTINGOVERRIDE_MIGRATION_LOCK_KEY)
    if not check_constraint_lists_members(
        ...
        return
    op.execute(
        "DELETE FROM settingoverride WHERE setting_class IN ('INVENTORY_SETTINGS')"
    )
    with op.batch_alter_table("settingoverride", schema=None) as batch_op:
        batch_op.alter_column(
            "setting_class",
