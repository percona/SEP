...
:class:`~app.extensions.apps.framework.script_source.ScriptSource`: it wires the hooks
``derive_script_routes`` needs around the surrounding snippets engine
(``app/extensions/snippets/``), so :data:`~app.extensions.apps.snippets.app.app` can declare
...
:meth:`~app.extensions.snippets.crud.SnippetManager.snippet_list_page` over
:attr:`~app.extensions.snippets.crud.SnippetManager.list_query_spec`) instead of fetching
...
* **The request-less listing.** ``load_script`` / ``list_scripts`` /
  ...
  their own request-less session (mirroring ``app/extensions/snippets/celery.py``).
...
from app.extensions.apps.framework.schema import AppSchema
from app.extensions.apps.framework.script_helpers import build_artifact_download_url
...
from app.extensions.snippets.deps import (
    build_snippet_execution_meta,
    get_snippet_list_query,
    validate_snippet_filename,
)
from app.extensions.snippets.list_query import SnippetListQuery
...
from app.extensions.snippets.schema import (
...
def build_snippet_source(snippet: Snippet) -> str:
    ...
    :func:`~app.extensions.snippets.deps.get_snippet_source`: it reads a
...
async def _list_scripts(
    ...
    :meth:`~app.extensions.snippets.crud.SnippetManager.snippet_list_page`, so the count and
    ...
    ordered — the shape
    :meth:`~app.extensions.apps.framework.list_query.InMemoryListQueryApplier.list_scripts`
...
async def _load_scripts(filenames: Sequence[str]) -> dict[str, SnippetScript]:
    ...
    :param filenames: The snippet filenames to resolve. Callers routing through
        :func:`~app.extensions.apps.framework.script_source.resolve_scripts` pass a
