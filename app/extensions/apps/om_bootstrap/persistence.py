...
:mod:`~app.extensions.apps.om_bootstrap.strategy` — ``models.py`` cannot, since Alembic
...
from app.extensions.apps.om_bootstrap import models
from app.extensions.apps.om_bootstrap.models import BootstrapRun
...
def parse_host_states(run: BootstrapRun) -> list[HostBootstrapState]:
    ...
    :return: One :class:`~app.extensions.apps.om_bootstrap.strategy.HostBootstrapState`
...
def dump_host_states(states: list[HostBootstrapState]) -> list[dict[str, Any]]:
    ...
    :class:`~app.extensions.apps.om_bootstrap.strategy.StepRecord` dump to ISO-8601
...
def parse_run_steps(run: BootstrapRun) -> list[StepRecord]:
    ...
    :attr:`~app.extensions.apps.om_bootstrap.models.BootstrapRun.run_steps`'s own
    docstring).

    :param run: The run whose ``run_steps`` document to parse.
    :return: One :class:`~app.extensions.apps.om_bootstrap.strategy.StepRecord` per
