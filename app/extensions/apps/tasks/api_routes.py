...
``app/extensions/api/router.py``. List and detail handlers proxy the tasks HTTP API
through ``TaskAPI``, mirroring the legacy Jinja routes in
``app.extensions.apps.tasks.routes``.
"""
...
from app.extensions.apps.tasks.models import (
...
async def tasks_api_detail(
    ...
    return TaskDetailResponse(
        task=resolve_task_actors(
            ExtensionsTaskResponse.model_validate(task.model_dump(mode="json")),
            username_map,
        ),
