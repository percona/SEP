...
from app.extensions.apps.checksums.deps import parse_checksums_task_args
from app.extensions.apps.checksums.models import (
    ChecksumsForm,
    coerce_target_list,
    OWNER,
)
...
from app.extensions.apps.framework.form_dsl import DSN_TABLE_DEFAULT
...
def _split_checksums_dsn_recursion(recursion_method: str) -> tuple[str, str]:
    ...
    :func:`~app.extensions.apps.checksums.spec.build_checksums_arg_prefix`). The
    create form keeps ``recursion_method="dsn"`` and ``dsn_table`` separately.

    :param recursion_method: The value produced by
        :func:`~app.extensions.apps.checksums.deps.parse_checksums_task_args`.
    :return: ``(recursion_method, dsn_table)`` suitable for :class:`~app.extensions.apps.checksums.models.ChecksumsForm`.
    """
...
def reconstruct_checksums_form(
    ...
    """Rebuild a :class:`~app.extensions.apps.checksums.models.ChecksumsForm` body from a legacy task.

    Combines ``task.name``, ``meta['target']``, inventory service resolution, and
    :func:`~app.extensions.apps.checksums.deps.parse_checksums_task_args`. Returns
