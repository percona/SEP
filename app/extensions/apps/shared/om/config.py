...
OM's tables go in the ``extensions`` database, isolated by a schema of their own rather
...
The mechanism is SQLAlchemy's **symbolic** schema, copied from what PMM Extensions already does
...
here: ``extensions_settings.DATABASE.SCHEMA_TRANSLATE_MAP`` (``app/core/db/config.py``) is
...
``app/extensions/migrations/env.py``, and, for the comparison ``alembic check`` runs,
...
#: translates it to a real name or to ``None`` via ``extensions_settings.DATABASE.
...
def om_schema(database: DatabaseOptions) -> str | None:
    ...
    :param database: The database options of the bind OM's tables live on —
        ``extensions_settings.DATABASE`` in every current caller.
