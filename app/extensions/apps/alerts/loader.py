...
from app.extensions.apps.alerts.models import AlertTemplate, ServiceType
...
def load_alert_templates(
...
) -> MappingProxyType[ServiceType, tuple[AlertTemplate, ...]]:
    ...
    returned mapping is a `MappingProxyType` and the per-service-type sequences
    are tuples, both immutable, to prevent callers from accidentally mutating
    the cached state.
    ...
    :rtype: MappingProxyType[ServiceType, tuple[AlertTemplate, ...]]
    :raises NotADirectoryError: If `definitions_dir` does not exist or is not a
        directory.
    :raises yaml.YAMLError: If a YAML file contains invalid YAML syntax.
    ...
    return MappingProxyType(
        {svc: tuple(templates) for svc, templates in result.items()}
    )


def get_alert_templates() -> MappingProxyType[ServiceType, tuple[AlertTemplate, ...]]:
    """Return alert templates loaded from the configured definitions directory.

    Use `extensions_settings.ALERT_DEFINITIONS_DIR` when set, otherwise fall back to
    ...
    """
    definitions_dir = (
        extensions_settings.ALERT_DEFINITIONS_DIR or DEFAULT_DEFINITIONS_DIR
    )
