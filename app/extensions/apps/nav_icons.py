...
(``pnpm --filter @pmm-extensions/shell codegen``), so a member added here needs a codegen
...
``app.extensions.apps.framework`` package: ``app.extensions.config`` types its ``App.NAV_ICON``
...
``app.extensions.deps``, which imports ``config``).
"""
