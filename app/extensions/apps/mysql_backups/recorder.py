...
:class:`~app.extensions.apps.mysql_backups.models.MysqlBackupRun`. The tasks service
...
from app.extensions.apps.meta_keys import SERVICE_ID_META_KEY, SERVICE_NAME_META_KEY
...
async def record_backup_run(
    ...
    upload_destination = canonical_backup_path(
        _coerce(reported.get("upload_destination"), str, "upload_destination")
    )
    location = canonical_backup_path(
        _coerce(reported.get("backup_dir"), str, "backup_dir")
    )
    ...
    async with get_async_session_maker()() as extensions_session:
        await MysqlBackupRunManager.get_or_create(
            extensions_session, record, filter_include={"task_history_id"}
        )
