...
from app.extensions.apps.framework import BaseTaskResponse
...
from app.extensions.apps.framework.rules import any_, F, not_
...
def repair_source_declaration(
    ...
    *,
    ...
    When ``source_transport`` is undeclared, ``catalogued_transport`` is forwarded
    to :func:`normalize_source_declaration` so a matching catalog S3/GCS value
    wins over field inference — the same preference the edit-form response
    builder applies.
    ...
    :param catalogued_transport: Optional object-store transport from the backup
        catalog; ignored when the stamp already declares ``source_transport``.
    :return: The repaired stamp, or ``None`` when no repair is owed.
    """
    repaired = _aligned_aes_declaration(
        normalize_source_declaration(stamp, catalogued_transport=catalogued_transport)
    )
