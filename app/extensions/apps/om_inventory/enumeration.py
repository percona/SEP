...
where none does. So hosts come from PMM Extensions' inventory nodes, filled by ``PMMSyncer``
...
:mod:`~app.extensions.apps.om_inventory.mapping` uses per service — name first, then
...
class InventoryHost:
    ...
    :param node_id: **PMM's** node id, which PMM Extensions' inventory stores as
...
async def list_inventory_nodes(inventory_api: RemoteAPI) -> list[dict]:
    """Return every node PMM Extensions' inventory holds.
...
def build_hosts(
    ...
    :param nodes: Raw node entries from PMM Extensions' inventory.
