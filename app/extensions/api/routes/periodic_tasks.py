...
Forward periodic-task CRUD to the Tasks sub-app through the PMM Extensions gateway so the
React frontend (``ScheduledTasksPanel``) reaches periodic-task list / create /
update / delete via PMM Extensions rather than calling ``/api/tasks`` directly. Every route
handler issues a single upstream call and forwards the response, mapping upstream
failures onto the PMM Extensions gateway error contract via
:func:`~app.extensions.api.proxy.reraise_upstream_tasks_error`. Create and update pass
the scheduling guard in :mod:`app.extensions.api.deps` before that write; list, delete
...
from app.extensions.api.deps import RequireSchedulableTask, RequireSchedulableUpdate
from app.extensions.api.openapi import (
    SCHEDULING_UNSUPPORTED_400_RESPONSE,
    UPSTREAM_TASKS_502_RESPONSE,
)
from app.extensions.api.proxy import reraise_upstream_tasks_errors
from app.extensions.deps import task_path, TaskAPI
...
async def list_periodic_tasks(
    ...
    """Return the upstream periodic-task page through the PMM Extensions gateway.
