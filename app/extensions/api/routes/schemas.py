...
frontend can populate schema-driven table selectors without bypassing the PMM Extensions
layer (see the non-bypass rule in ``app/extensions/api/router.py``).
"""
...
async def list_schema_tables(
    ...
    """Return tables for a schema via the PMM Extensions gateway.
