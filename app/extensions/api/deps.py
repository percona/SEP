...
"""Define dependencies for the PMM Extensions level ``/api/extensions/`` JSON routes.

They live here rather than in ``app/extensions/deps.py`` because the scheduling guard
resolves the owning app through ``get_app_registry()``, and the registry module
imports ``app.extensions.deps``.
"""
...
from app.extensions.deps import get_task_by_name, TaskAPI
