...
class RestoreForm(TaskFormModel):
    ...
    (``alert_on_fail`` is ``Hidden``, off-schema). The presentation default for
    ``task_name`` is applied in ``restore/schema.py`` after derivation so this form
    does not redeclare the shared identity fields. This form is never validated as
    a request body.
    """
