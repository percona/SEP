...
one POM exists to describe.

Forward-only, rather than editing the migration that created the table: the table is
already applied wherever POM has run, and three nullable-with-default columns are
...
from app.sep.pom.config import pom_schema
...
def _schema() -> str | None:
    """Return the real schema POM's tables live in on this bind.
    ...
    """
    return pom_schema(sep_settings.DATABASE)
...
def upgrade() -> None:
    ...
    existing = {
        ...
        for column in sa.inspect(op.get_bind()).get_columns(
            "discovery_run", schema=schema
        )
    }
    with op.batch_alter_table("discovery_run", schema=schema) as batch_op:
