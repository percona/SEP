...
"""Define the POM Discovery persistence models.

Three tables, and the split between them is the design:

``pom.host`` and ``pom.service``
...
# Table names are short -- ``host``, ``service``, ``discovery_run`` -- because the
# schema is what qualifies them. ``pom.service`` is a different table from SEP
...
# gives each SQLite connection a real ``pom`` schema with ``ATTACH`` rather than
...
class ObservedEntity(SQLModel):
    ...
    * the upsert lists the columns it owns explicitly. Nothing here is user-writable
    ...
    :param first_seen_at: When POM first wrote a row for this entity.
...
class PomHost(ObservedEntity, table=True):
    """Record one host, whether or not a database was found on it.

    Keyed on **PMM's** node id. POM is not the system of record for identity here,
    ...
    id, POM gets a second row and the old one stays until retention prunes it. The
    usual answer -- a POM-minted id plus a natural key to recognise the machine across
    ...
    Class named ``PomHost`` rather than ``Host`` even though the *table* is
    ``pom.host``: the schema qualifies the table, but SQLModel keeps one registry for
    ...
    __table_args__ = (
        ...
        # POM's migrations are hand-written anyway (autogenerate is off for them).
        Index(
...
class PomService(ObservedEntity, table=True):
    ...
    ``pom`` schema takes no foreign keys at all: each app that owns migrations is an
    ...
    :param service_id: PMM's service id, the primary key. ``text`` for the same
        reason as :attr:`PomHost.node_id`.
    ...
    __table_args__ = (
        ...
        # index would ship into the real ``pom`` schema called
        # ``ix_pom_schema_service_node_id``.
        Index("ix_pom_service_node_id", "node_id"),
        {"schema": POM_SCHEMA},
    )
    ...
    node_id: str = SQLField(
        sa_column=Column(
            ...
            ForeignKey(f"{POM_SCHEMA}.host.node_id", ondelete="CASCADE"),
            nullable=False,
        )
    )
...
class ProbeRun(BaseUUIDSQLModel, table=True):
    ...
    # (``app/sep/pom/config.py``). Never a literal: SQLite has no schemas, so a real
    ...
    # hard-coded ``pom`` would escape and then collide across workers.
