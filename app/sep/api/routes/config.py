...
"""Define the ``/api/sep/admin/config/export`` YAML download endpoint.

Aggregate every settings class wired into the live SEP admin surface plus
``TasksSettings`` from the Tasks sub-app so the React Settings page can
download the merged effective configuration through SEP rather than calling
each sub-app separately. A Tasks fan-out failure re-raises as
:class:`~app.core.exceptions.HTTPBadGatewayException` (``502`` JSON) — no
partial export is returned.
"""
...
from app.core.settings_override.api import build_settings_class_values
...
from app.sep.deps import IsApiAdmin, SessionDep, TaskAPI

router = APIRouter(dependencies=[IsApiAdmin])
