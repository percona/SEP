...
from app.tasks.models import GeneratedTask, Task, TaskHistoryResponse
...
async def get_alters_task_history(
    task_history_id: int, tasks_api: TaskAPI
) -> TaskHistoryResponse:
    """Fetch and validate a task history for the Alters plugin.

    This function retrieves a task history by its ID from the Tasks API and optionally
    validates that it is owned by the Alters plugin. If the task history does not exist
    ...
    :raises HTTPNotFoundException: If the task history is not found or the validation
        fails.
    """
    return await get_task_history(tasks_api, task_history_id, "alters")
