...
def parse_host_port(host_entry: str) -> tuple[str, int]:
    """Split a ``host[:port]`` entry into ``(host, port)`` with default 3306."""
    if ":" in host_entry:
...
def _query_server_info(cursor: DictCursor) -> dict[str, Any]:
    """Return basic MySQL server identity + read-only mode."""
    try:
        cursor.execute(
...
def _query_repl_info(cursor: DictCursor) -> dict[str, Any]:
    ...
    for query in ("SHOW REPLICA STATUS", "SHOW SLAVE STATUS"):
        ...
        io = row.get("Replica_IO_Running", row.get("Slave_IO_Running"))
        sql = row.get("Replica_SQL_Running", row.get("Slave_SQL_Running"))
        ...
        return {
            "source_host": row.get("Master_Host", row.get("Source_Host")),
            "source_port": row.get("Master_Port", row.get("Source_Port")),
            "source_server_id": row.get(
                "Master_Server_Id", row.get("Source_Server_Id")
            ),
            "source_uuid": row.get("Master_UUID", row.get("Source_UUID")),
            "io_running": io,
            "sql_running": sql,
            "seconds_behind": row.get(
                "Seconds_Behind_Master", row.get("Seconds_Behind_Source")
            ),
...
def _query_cluster_info(cursor: DictCursor) -> dict[str, Any]:
    """Return PXC cluster info, or ``{}`` when not in a cluster."""
    try:
        cursor.execute(
...
def collect_host(
    ...
    *,
    ...
    """Collect topology data for one MySQL host. Returns the per-host record."""
...
def _emit(obj: dict[str, Any]) -> None:
    """Write one NDJSON line to stdout and flush so consumers see it now."""
    sys.stdout.write(json.dumps(obj, separators=(",", ":")) + "\n")
    sys.stdout.flush()


def run(config: dict[str, Any]) -> int:
    """Run the topology collection. Returns the count of failed hosts."""
