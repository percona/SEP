...
from app.sep.plugins.checksums.deps import parse_checksums_task_args
from app.sep.plugins.checksums.spec import DEFAULT_RECURSION_DSN_TABLE
from app.sep.plugins.framework.form_backfill_inventory import meta_service_hints

if TYPE_CHECKING:
    from app.sep.plugins.framework.form_backfill import FormBackfillContext
...
def _split_checksums_dsn_recursion(recursion_method: str) -> tuple[str, str]:
    ...
    :func:`~app.sep.plugins.checksums.spec.build_checksums_arg_prefix`). The
    create form keeps ``recursion_method="dsn"`` and ``dsn_table`` separately.

    :param recursion_method: The value produced by
        :func:`~app.sep.plugins.checksums.deps.parse_checksums_task_args`.
    :return: ``(recursion_method, dsn_table)`` suitable for :class:`~app.sep.plugins.checksums.models.ChecksumsForm`.
    """
...
def reconstruct_checksums_form(
    ...
    """Rebuild a :class:`~app.sep.plugins.checksums.models.ChecksumsForm` body from a legacy task.

    Combines ``task.name``, ``meta['target']``, inventory service resolution, and
    :func:`~app.sep.plugins.checksums.deps.parse_checksums_task_args`. Returns
