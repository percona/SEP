...
:class:`~app.sep.plugins.framework.script_source.ScriptSource`: it wires the six
hooks ``derive_script_routes`` needs around the existing snippets engine
(``app/sep/snippets/``), so :data:`~app.sep.plugins.snippets.app.app` can declare
...
from app.sep.plugins.snippets.models import build_snippet_response, SnippetResponse
...
def build_snippet_source(snippet: Snippet) -> str:
    ...
    :func:`~app.sep.plugins.snippets.deps.get_snippet_source`: it reads a
