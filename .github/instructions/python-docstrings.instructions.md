...
Model fields are constructor parameters — document with `:param:`, NOT `:cvar:`. The field annotation supplies the type, so a `:type:` is optional. Applies to every `BaseModel` / `SQLModel` subclass. Only fields genuinely wrapped in `ClassVar[...]` use `:cvar:` — those *are* class variables, and `:cvar:` is the only correct form for them, but the form is owed only when the member needs documenting at all. A member whose name already *is* its documentation takes no entry: `:cvar NODE: The node entity.` for `NODE = "node"` restates the identifier, and four such lines push the members off the reader's screen. Document the **enum** — what the value space means, where it is persisted, what a member's string has to match — and add a `:cvar:` only for a member carrying something its name does not (a wire value differing from the member name, a deprecation, a default, a load-bearing ordering); `:type:` is reserved for `:param:`, and a class variable that documents its type uses `:vartype:`. The same rule applies to `@dataclass` (including `frozen=True` / `slots=True`): annotated fields are constructor parameters — use `:param:`, never `:ivar:` / `:cvar:` (those are reserved for true `ClassVar`s or `__post_init__`-assigned attributes).

**A model docstring is published API documentation.** Pydantic serializes a model's class docstring into the schema `description`, so it ships verbatim into `frontend/packages/api/specs/*.json` and into the generated TypeScript — a client author reads it with no view of the server. A `:param:` block therefore must not name **private implementation methods**, validation sequencing, or any server-side mechanism the reader cannot call. Keep the client-facing half — what the field means, what values it takes, when it is withheld — and move the mechanics into the private method's own docstring, which is not serialized.
...
Spell it **`:return:`, singular**. `:returns:` is also valid rST; this project standardises on the singular form.
...
That exemption is for functions needing **no** explanation. A function whose body opens with a block of comments has already decided it needs one, and filed it in the surface that cannot render, is not read by `help()`, and is skipped by every docstring check — that prose is a docstring. The tell is position: a comment between the `def` line and the first statement describes the *function*; a comment beside a mid-body statement describes the *statement* and is legitimate. Four or more contiguous own-line comments opening an undocumented function is the threshold; a block that is only tool directives (`noqa`, `type:`) addresses a checker, not a reader, and is exempt. Moving the prose is also when its redundancy against a neighbouring docstring surfaces.
