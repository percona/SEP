...
- A new function under `app/sep/` reads `tasks_settings.*` or `inventory_settings.*`, or imports `NomadExecutor` / `CeleryExecutor` / `BaseExecutor`.
- A new entry in `app/sep/db/seed.py::get_system_periodic_tasks()` whose task operates on tasks/inventory state. `app/sep/` seeds via `app/sep/db/seed.py::get_system_periodic_tasks()`, paired with `app/sep/periodic_tasks.py`; `app/tasks/` pairs `app/tasks/celery.py` with `app/tasks/db/seed.py::SYSTEM_PERIODIC_TASKS`. Use the matching pair. `app/inventory/` seeds no periodic tasks at all — a schedule proposed for it needs a new pair, not an existing one.
...
- **A new per-app block in a shared core registry.** A shared module that grows one import, one `if app == …`, or one hardcoded entry per app is an accretion smell wherever it appears: the app is registering itself somewhere it doesn't own, instead of contributing through a seam that does. SEP has that seam — `app/sep/apps/framework/registry.py`, from which the Celery `include` list (`build_celery_include()`) and the app-owned periodic schedules are both derived, so neither needs a per-app edit. Adding a *new* per-app block to a shared module must be **justified** ("why not an app-package seam?") even when a precedent block already lives there: precedent, not licence. Flag-and-justify, not a hard block — no retro-move demanded. **Celery is not such a case**: every app-domain task (`@owned_by(<app>)`) has an app-package home today — `app/sep/apps/{alerts,atw,report}/celery.py` — so a new `@owned_by` group landing in a shared module is a defect to fix, not a smell to justify. (`app/sep/snippets/celery.py` is not one of those homes and is not a counter-example either: its task is deliberately unowned, carries no `@owned_by`, and is named in `STATIC_CELERY_INCLUDE` so it survives an image that never mounts the snippets app.)
