...
`app/sep/apps/<name>/` with `routes.py`, `deps.py`, optional `models.py`. Registration in `settings.yaml` under `SEP.PLUGINS`. Flag plugins that put dep aliases in `routes.py`/`models.py` or scatter helpers into ad-hoc module names.
