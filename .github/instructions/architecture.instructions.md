...
| `app/tasks/` | Task execution, the Nomad and Celery executors, executor-config-driven side effects, executor periodic tasks | `tasks_settings` |
| `app/extensions/` | Web UI, apps, OAuth, alerts, cross-cutting concerns not owned by a single domain | `extensions_settings` |

Red flags:

- A new function under `app/extensions/` reads `tasks_settings.*` or `inventory_settings.*`, or imports `NomadExecutor` / `CeleryExecutor` / `BaseExecutor`.
- A new entry in `app/extensions/db/seed.py::get_system_periodic_tasks()` whose task operates on tasks/inventory state. `app/extensions/` seeds via `app/extensions/db/seed.py::get_system_periodic_tasks()`, paired with `app/extensions/periodic_tasks.py`; `app/tasks/` pairs `app/tasks/celery.py` with `app/tasks/db/seed.py::SYSTEM_PERIODIC_TASKS`. Use the matching pair. `app/inventory/` seeds no periodic tasks at all — a schedule proposed for it needs a new pair, not an existing one.
- A test for a tasks-flavored periodic task landing under `tests/app/extensions/` instead of `tests/app/tasks/`.
- A module in `app/tasks/` or `app/inventory/` hard-codes a specific app's parser, schema, magic strings, or enrichment for one app — even wrapped as "generic infra". The tell: a helper returning app-specific content for one app and `None` for the rest. App knowledge stays in the app's package; the generic service holds only the registration/hook mechanism. (The inverted *import* — `app/tasks` or `app/inventory` importing `app.extensions.apps` — is caught mechanically; app knowledge **copied** into the generic service imports nothing and stays a reviewer call.)
- **A new per-app block in a shared core registry.** A shared module that grows one import, one `if app == …`, or one hardcoded entry per app is an accretion smell wherever it appears: the app is registering itself somewhere it doesn't own, instead of contributing through a seam that does. PMM Extensions has that seam — `app/extensions/apps/framework/registry.py`, from which the Celery `include` list (`build_celery_include()`) and the app-owned periodic schedules are both derived, so neither needs a per-app edit. Adding a *new* per-app block to a shared module must be **justified** ("why not an app-package seam?") even when a precedent block already lives there: precedent, not licence. Flag-and-justify, not a hard block — no retro-move demanded. **Celery is not such a case**: every app-domain task (`@owned_by(<app>)`) has an app-package home today — `app/extensions/apps/{alerts,atw,report}/celery.py` — so a new `@owned_by` group landing in a shared module is a defect to fix, not a smell to justify. (`app/extensions/snippets/celery.py` is not one of those homes and is not a counter-example either: its task is deliberately unowned, carries no `@owned_by`, and is named in `STATIC_CELERY_INCLUDE` so it survives an image that never mounts the snippets app.)

## App config never lives in a core module

App operational settings belong to the app, not to a core module. `app/extensions/config.py` / `ExtensionsSettings` and `app/core/**` are core surfaces mounted by the whole application, so a field, type, default, or import carrying app-specific knowledge there makes **core depend on an app package** — the wrong-direction dependency. Read the section straight off YAML/env in an app-owned `app/extensions/apps/<app>/config.py`, imported by consumers at call time.
...
- **Flag:** `ExtensionsSettings.ATW: AtwSettings = AtwSettings()` where `AtwSettings` is defined in `app/extensions/apps/atw/` — whether or not it cycles.
- **Not a violation:** a genuinely cross-app / core setting (`settings.PMM`, a shared timeout) as an `ExtensionsSettings` field; and an app importing *from* `app.extensions.config` — app→core is the allowed direction.

Separately, **any `ExtensionsSettings` field typed with a class from an app *package* does cycle**: importing `app.extensions.apps.<app>.config` first executes `app/extensions/apps/<app>/__init__.py`, which imports the app object for registry discovery → `api_routes` → … → `app/extensions/deps.py` → `extensions_settings`, only partially initialized at settings-construction time. Every app package has this `__init__.py` shape, so no import discipline inside the leaf `config.py` avoids it. Reason about an import edge into a settings-construction-time module from **what importing the target actually executes**, not from the leaf module's own import list — and a precedent claim must share the structural property, not just the `app/extensions/apps/…` path prefix (`app/extensions/apps/nav_icons.py` is a leaf module whose `__init__.py` has zero imports).
...
- **A rank-ordered enum refuses a foreign operand — it does not return `NotImplemented`.** When ordering carries authorization meaning, `NotImplemented` is not a neutral decline: Python reads it as delegation to the reflected operation, and a `str`/`int` base answers, so `EDITOR >= "admin"` can be **True** while `EDITOR >= UserRole.ADMIN` is False. Raise `TypeError` for a non-member from one shared ranking helper (`UserRole` in `app/core/auth/models.py`), and check that *all four* of `__lt__`/`__le__`/`__gt__`/`__ge__` are covered — on a plain `Enum` `functools.total_ordering` derives the other three from `__lt__`, which is how `UserRole` covers them, but it is a **no-op on a `str`- or `int`-mixin enum**, where the mixin already supplies all four and the decorator finds nothing to fill; there they must be written out. Pin the operand *type* in tests, not just member pairs.
- **A `field_validator` on a `table=True` model is dead code on the construction path.** SQLModel bypasses Pydantic validation in `__init__` for table models, so `Foo(name="abc")` stores the value exactly as passed. Nothing raises; the coercion silently does not happen, while the docstring reads like a guarantee. Put the invariant where it runs — a `TypeDecorator.process_bind_param`, a CRUD manager method, or a non-table request/response model. A table model's validator is reached only through an explicit `Foo.model_validate(...)` or `validate_assignment`; if you are relying on one of those, say so in the docstring.
...
`app/extensions/apps/<name>/` with `routes.py`, `deps.py`, optional `models.py`. Registration in `settings.yaml` under `EXTENSIONS.APPS`. Flag apps that put dep aliases in `routes.py`/`models.py` or scatter helpers into ad-hoc module names.
