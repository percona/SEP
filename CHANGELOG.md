...
- SEP-818: Add PostgreSQL and SQLite expression indexes on `taskhistory.execution_request->'task'`/`'target'` so dispatch dedup and task-history filter queries use index scans instead of the narrowed candidate set
