...
export function useExecutionEvents(taskId: string | undefined) {
  return useQuery<ExecutionEvent[]>({
    ...
    queryFn: async () => {
      ...
      return [
        { id: '1', type: 'started' as const, timestamp: new Date().toISOString(), message: 'Execution started' },
        { id: '2', type: 'completed' as const, timestamp: new Date().toISOString(), message: 'Execution completed' },
      ];
    },
    placeholderData: [],
  });
