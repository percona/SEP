...
async function mockReportRoutes(page: Page) {
  await page.route('**/api/**', (route) => {
    ...
    if (pathname === '/api/report/config') {
      return route.fulfill({ json: MOCK_CONFIG });
    }
    if (pathname === '/api/report/generate/json' && req.method() === 'GET') {
      return route.fulfill({ json: MOCK_REPORT });
    }
    if (pathname === '/api/report/generate/pdf' && req.method() === 'POST') {
    ...
    if (pathname === '/api/report/upload' && req.method() === 'POST') {
