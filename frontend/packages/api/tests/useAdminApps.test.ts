...
import { appStateErrorMessage, isTransitional } from '../src/hooks/useAdminApps';
...
describe('appStateErrorMessage', () => {
  ...
  it('falls back to a generic message when 404 has no detail', () => {
    const error = new ApiError({ kind: 'http', status: 404, message: 'HTTP 404' });
    expect(appStateErrorMessage(error)).toBe('That app no longer exists.');
  });

  it('reports an admin-access message for a 403', () => {
    const error = new ApiError({ kind: 'http', status: 403, message: 'Forbidden' });
  ...
  it('reports a credentials hint for a 401 (missing bearer token)', () => {
    const error = new ApiError({ kind: 'http', status: 401, message: 'Unauthorized' });
