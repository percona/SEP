...
describe('typed-client — 401 recovery', () => {
  const mintOnce = (token: string) => {
    const minter = vi.fn(async () => ({ access_token: token, expires_in: 300 }));
  ...
  it('replays a request body — `fetch` consumed the original stream', async () => {
    ...
    server.use(
      http.post('http://localhost/api/apps/inventory/sync/', async ({ request }) => {
        const auth = request.headers.get('Authorization');
  ...
  it('does not attempt recovery when a minting endpoint itself 401s', async () => {
    const minter = mintOnce('fresh');
    const onUnauth = vi.fn();
    setOnUnauthorized(onUnauth);
