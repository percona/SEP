...
describe('postSession — ambient auto-login', () => {
  it('returns the slim token shape on a 200 from /oauth/session', async () => {
    server.use(
      http.post(`${BASE}/api/oauth/session`, () =>
        HttpResponse.json({ access_token: 'ambient', expires_in: 300 }),
      ),
    );

    await expect(postSession()).resolves.toEqual({ access_token: 'ambient', expires_in: 300 });
  });
...
describe('apiClient — pluggable token minter', () => {
  it('recovers through the registered minter instead of /oauth/refresh', async () => {
    ...
    // The embedded-host wiring: exchange the host session cookie, no refresh
