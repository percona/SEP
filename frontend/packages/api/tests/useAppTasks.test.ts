...
describe('isBackendUnavailable (mock-fallback gate)', () => {
  ...
  it('does not mask timeout, canceled, or unknown kinds as backend-down', () => {
    expect(isBackendUnavailable(new ApiError({ kind: 'timeout', message: 't' }))).toBe(false);
    expect(isBackendUnavailable(new ApiError({ kind: 'canceled', message: 'c' }))).toBe(false);
    expect(isBackendUnavailable(new ApiError({ kind: 'unknown', message: '?' }))).toBe(false);
  });
  ...
  it('does not treat http error with missing status as backend-down', () => {
    ...
    expect(isBackendUnavailable(new ApiError({ kind: 'http', message: 'mystery' }))).toBe(false);
  });
});
