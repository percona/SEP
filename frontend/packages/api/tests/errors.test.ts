...
describe('parseFieldErrors', () => {
  it('strips a leading "body" segment from FastAPI request-body validation locs', () => {
    const error = http422([
      { loc: ['body', 'limit'], msg: 'ensure this value is greater than 0', type: 'greater_than' },
    ]);
  ...
  it('keeps locs without a "body" prefix (manual model_validate paths)', () => {
    const error = http422([{ loc: ['sleep'], msg: 'field required', type: 'missing' }]);
    expect(parseFieldErrors(error)).toEqual([{ path: 'sleep', message: 'field required' }]);
  });

  it('joins nested object and array-index segments into a dot path', () => {
    const error = http422([
      { loc: ['body', 'source', 'host'], msg: 'invalid host', type: 'value_error' },
      { loc: ['body', 'items', 0, 'name'], msg: 'too short', type: 'string_too_short' },
    ]);
    expect(parseFieldErrors(error)).toEqual([
  ...
  it('yields an empty path for a body-level error so callers can surface it', () => {
    const error = http422([{ loc: ['body'], msg: 'value is not a valid dict', type: 'dict_type' }]);
    expect(parseFieldErrors(error)).toEqual([{ path: '', message: 'value is not a valid dict' }]);
  });

  it('falls back to a generic message when msg is missing or empty', () => {
    const error = http422([{ loc: ['body', 'name'] }, { loc: ['body', 'age'], msg: '' }]);
    expect(parseFieldErrors(error)).toEqual([
  ...
  it('accepts a raw payload object as well as an ApiError', () => {
    expect(parseFieldErrors({ detail: [{ loc: ['body', 'x'], msg: 'bad' }] })).toEqual([
      { path: 'x', message: 'bad' },
    ]);
  });
  ...
  it('returns [] when there is no recognizable detail array', () => {
    ...
    expect(parseFieldErrors(new ApiError({ kind: 'network', message: 'offline' }))).toEqual([]);
