 ...
 *
 * Fetches ``GET /api/sep/admin/config/export`` through ``apiClient`` so the
...
const CONFIG_EXPORT_PATH = '/sep/admin/config/export';
...
export function useConfigExport() {
  return useMutation<void, ApiError, void>({
    mutationFn: async () => {
      ...
      const filename = filenameFromContentDisposition(header) ?? DEFAULT_FILENAME;
