...
export type AppLifecycleState = 'ENABLED' | 'DISABLED' | 'ENABLING' | 'DISABLING';

/** The transitional states a toggle may request; the backend rejects the
 *  terminal `ENABLED`/`DISABLED` edges with 409. */
export type TransitionalState = Extract<AppLifecycleState, 'ENABLING' | 'DISABLING'>;
...
export function useAdminApps(options: { enabled?: boolean } = {}) {
  return useQuery<AdminApp[], ApiError>({
    ...
    refetchInterval: (query) => {
      ...
      return apps?.some((app) => isTransitional(app.lifecycle_state)) ? TRANSITION_POLL_MS : false;
    },
  });
}
 ...
 */
function applyTransition(queryClient: QueryClient, result: AppStateResult): void {
  queryClient.setQueryData<AdminApp[]>(ADMIN_APPS_QUERY_KEY, (apps) =>
    apps?.map((app) =>
      app.app_key === result.app_key
        ? { ...app, enabled: result.enabled, lifecycle_state: result.lifecycle_state }
        : app,
    ),
  );
 ...
 */
export function appStateErrorMessage(error: ApiError | null | undefined): string | null {
  if (!error) {
    return null;
  }
