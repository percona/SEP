...
describe('ResultsPane recorded arguments', () => {
  ...
  it('renders the masked arguments in the collapsed summary', async () => {
    mockedApi.get.mockResolvedValue(paginated([executionWithArgs({ masked_args: MASKED_ARGS })]));
  ...
  it('wraps the arguments in the body while the summary keeps them on one line', async () => {
    ...
    mockedApi.get.mockResolvedValue(paginated([executionWithArgs({ masked_args: longArgs })]));
  ...
  it('shows an empty state when the execution recorded no arguments', async () => {
    mockedApi.get.mockResolvedValue(
      paginated([executionWithArgs({ masked_args: null, args_withheld: false })]),
    );
...
function routeGet(routes: {
  ...
  mockedApi.get.mockImplementation((url: string, config?: { params?: { offset?: number } }) => {
...
const selectAllToggle = () => screen.getByLabelText(SELECT_ALL_LABEL) as HTMLInputElement;
 ...
 *
 * MUI reports the mixed state as `aria-checked="mixed"` and leaves the native
 * `checked` property false, so asserting the ARIA state pins the contract a
 * screen reader actually consumes rather than MUI's own markup.
 */
const toggleState = () =>
  selectAllToggle().getAttribute('aria-checked') === 'mixed'
    ? 'mixed'
...
function pagesByOffset(pages: Record<number, unknown[]>, total: number) {
  return (offset: number) => ({ items: pages[offset] ?? [], total, offset, limit: 20 });
}
...
describe('ResultsPane select all on the page', () => {
  ...
  it('selects every finished execution on the page and skips the unfinished ones', async () => {
    ...
    expect(
      (screen.getByRole('button', { name: /Send to support case/i }) as HTMLButtonElement).disabled,
    ).toBe(false);
  });
  ...
  it('renders the toggle indeterminate while only some finished rows are selected', async () => {
    routeGet({
      executions: { items: [FINISHED_EXECUTION, STALE_EXECUTION], total: 2, offset: 0, limit: 20 },
    });
  ...
  it('disables the toggle when the page holds no finished execution', async () => {
    routeGet({
      executions: { items: [RUNNING_EXECUTION], total: 1, offset: 0, limit: 20 },
    });
  ...
  it('leaves another page selected when deselecting the current one', async () => {
    routeGet({
      executions: pagesByOffset(
        { 0: [FINISHED_EXECUTION, RUNNING_EXECUTION], 20: [SECOND_PAGE_EXECUTION] },
        40,
      ),
    });
  ...
  it('labels an execution the toggle selected after the user pages away from it', async () => {
    routeGet({
      executions: pagesByOffset({ 0: [FINISHED_EXECUTION], 20: [SECOND_PAGE_EXECUTION] }, 40),
    });
    ...
    fireEvent.click(screen.getByRole('button', { name: /Send to support case/i }));
...
describe('ResultsPane — write access', () => {
  ...
  it('renders the send and re-send actions for a session that may mutate', async () => {
    routeGet({
      executions: { items: [FINISHED_EXECUTION], total: 1, offset: 0, limit: 20 },
      sendJobs: { items: [failedJob], total: 1, offset: 0, limit: 50 },
    });

    renderPane(<ResultsPane incidentId="inc-1" />);

    await waitFor(() => expect(screen.getByText('Send history')).toBeTruthy());
    expect(screen.getByRole('button', { name: /Send to support case/i })).toBeInTheDocument();
  ...
  it('renders no send, re-send or selection controls for a non-admin', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Send history')).toBeTruthy());
    expect(screen.queryByRole('button', { name: /Send to support case/i })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Re-send' })).not.toBeInTheDocument();
    expect(screen.queryByLabelText('Select diag/slow-query.sh')).not.toBeInTheDocument();
...
describe('ResultsPane live log', () => {
  ...
  function streamRequests(): string[] {
    return fetchSpy.mock.calls
      .map(([input]) => String(input))
      .filter((url) => url.includes('/stream-logs/'));
  }
  ...
  it('streams the log of a running execution before any of it is captured', async () => {
    ...
    await waitFor(() => {
      expect(
        streamRequests().some((url) =>
          url.endsWith(`/stream-logs/${RUNNING_EXECUTION.task_history_id}`),
        ),
      ).toBe(true);
    });
