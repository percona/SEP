...
const CATEGORY_LISTING = [
  {
    ...
    snippet_count: 1,
    snippets: [
      { name: 'diag/slow-query.sh', title: 'Slow Query Diagnostics', description: 'Tagged.' },
    ],
  },
];
...
describe('CollectPane snippet search', () => {
  ...
  it('reports how many matches the fetched page left out', async () => {
    ...
    const notice = await screen.findByText(/Showing the first 1 of 137 snippets/, undefined, {
      timeout: 3000,
    });
    expect(notice).toHaveTextContent('Type more of the name or description to narrow the results.');
  });
