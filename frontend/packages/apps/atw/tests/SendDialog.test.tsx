    ...
    if (url.startsWith('/apps/atw/config/')) {
      return Promise.resolve({
        data: omitAvailabilityField
          ...
          : { send_disabled_reasons: [], case_search_available: caseSearchAvailable },
      });
    }
...
function caseSearchTerms(): string[] {
  return mockedApi.get.mock.calls
    .filter(([url]: [string]) => url.startsWith('/apps/atw/case-search/'))
    .map(([, config]: [string, { params: { term: string } }]) => config.params.term);
}
...
describe('SendDialog', () => {
  ...
  it('prefills the case reference from the incident', () => {
    ...
    const field = screen.getByLabelText(/Support case reference/i) as HTMLInputElement;
  ...
  it('states that the bundle carries both output files and logs', () => {
    ...
    expect(screen.getByText(/output files and captured logs are bundled/i)).toBeTruthy();
  });
  ...
  it('disables Send when the selection is empty', () => {
    ...
    expect(screen.getByText(/None of the selected executions still exist/i)).toBeTruthy();
  });
  ...
  it('posts the selection, then follows the job to success', async () => {
    ...
    expect(mockedApi.post).toHaveBeenCalledWith('/apps/atw/incidents/inc-1/send-jobs/', {
      case_ref: 'CS0042',
  ...
  it('surfaces the recorded error and offers Send again on failure', async () => {
    ...
    mockedApi.get.mockResolvedValue({
      data: sendLog({
        status: 'failed',
        finished_at: '2026-07-24T10:01:00Z',
        detail: { error: 'Bundle exceeded the configured 30 MiB limit while being built.' },
      }),
    });
...
describe('SendDialog case search', () => {
  ...
  it('offers the cases the provider matched for the typed term', async () => {
    ...
    expect(await screen.findByRole('option', { name: /CS0001/ }, { timeout: 3000 })).toBeTruthy();
  ...
  it('puts the picked case reference in the field', async () => {
    ...
    await userEvent.type(caseRefField(), 'CS00');
    await userEvent.click(await screen.findByRole('option', { name: /CS0002/ }, { timeout: 3000 }));
  ...
  it('offers no options and no empty state when the search is unavailable', async () => {
    ...
    await userEvent.type(caseRefField(), 'CS00');
    await waitFor(() => expect(caseSearchTerms()).toHaveLength(1), { timeout: 3000 });
  ...
  it('distinguishes a search that matched nothing from one that could not run', async () => {
    ...
    expect(await screen.findByText(/No matching case/i, {}, { timeout: 3000 })).toBeTruthy();
  });
