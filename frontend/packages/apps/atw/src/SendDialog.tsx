...
const NO_MATCH_HELPER = 'No matching case. The reference is sent exactly as typed.';
...
export function SendDialog({
  ...
  const searchQuery = useAtwCaseSearch(debouncedCaseRef, Boolean(config?.case_search_available));
  ...
  const searchFoundNothing = search?.available === true && !isSearching && matches.length === 0;
        ...
        {succeeded ? (
          ...
          <>
            <DialogContentText sx={{ mb: 2 }}>
              The selected executions&apos; output files and captured logs are bundled and attached
              to the support case.
            </DialogContentText>
            ...
            <Autocomplete<AtwCaseMatch, false, false, true>
              ...
              onChange={(_event, value) =>
                setCaseRef(typeof value === 'string' ? value : (value?.reference ?? ''))
              }
              getOptionLabel={(option) => (typeof option === 'string' ? option : option.reference)}
              ...
              getOptionKey={(option) => (typeof option === 'string' ? option : option.reference)}
              loading={isSearching}
              clearOnBlur={false}
