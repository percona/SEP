 ...
 */
export function sendJobDetail(job: AtwSendLog | undefined | null): AtwSendLogDetail {
 ...
 */
export function useAtwIncidents(page: AtwPageParams = { offset: 0, limit: ATW_PAGE_SIZE }) {
  return useQuery<AtwPage<AtwIncident>>({
    queryKey: [...incidentsKey, page],
    placeholderData: keepPreviousData,
    queryFn: async () => {
      const { data } = await apiClient.get<AtwPage<AtwIncident>>(`${ATW_BASE}/incidents/`, {
...
export function useAtwIncident(incidentId: string | undefined) {
  return useQuery<AtwIncident>({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<AtwIncident>(`${ATW_BASE}/incidents/${incidentId}`);
      return data;
    },
  });
}
...
export function useCreateAtwIncident() {
  ...
  return useMutation<AtwIncident, Error, AtwIncidentWrite>({
    mutationFn: async (body) => {
      const { data } = await apiClient.post<AtwIncident>(`${ATW_BASE}/incidents/`, body);
      return data;
    },
    onSuccess: () => {
...
export function useUpdateAtwIncident() {
  const queryClient = useQueryClient();
  return useMutation<AtwIncident, Error, { incidentId: string; body: AtwIncidentUpdate }>({
...
export function useAtwMergedSchema(snippetFilenames: string[]) {
  ...
  return useQuery<AtwMergedSchema>({
    ...
    staleTime: ATW_STALE_TIME_MS,
    queryFn: async () => {
      const { data } = await apiClient.get<AtwMergedSchema>(`${ATW_BASE}/execution-schema/`, {
...
export function useAtwBatchExecute(incidentId: string) {
  ...
  return useMutation<AtwBatchExecuteResponse, Error, AtwBatchExecuteWrite>({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: incidentExecutionsKey(incidentId) });
    },
  });
}
...
export function useAtwCaseSearch(term: string, enabled: boolean) {
  ...
  return useQuery<AtwCaseSearchResponse>({
    ...
    queryFn: async () => {
      try {
        const { data } = await apiClient.get<AtwCaseSearchResponse>(`${ATW_BASE}/case-search/`, {
          params: { term: trimmed },
        });
        return data;
      } catch {
...
export function useAtwSendJob(incidentId: string, jobId: string | null) {
  return useQuery<AtwSendLog>({
    ...
    refetchInterval: (query) => (isSendJobActive(query.state.data) ? SEND_JOB_POLL_MS : false),
  });
}
...
export function useAtwSendJobs(incidentId: string | undefined) {
  return useQuery<AtwPage<AtwSendLog>>({
    queryKey: incidentId ? sendJobsKey(incidentId) : ['atw', 'incidents', 'send-jobs'],
    enabled: Boolean(incidentId),
    queryFn: async () => {
    ...
    refetchInterval: (query) =>
      query.state.data?.items.some((row) => isSendJobActive(row)) ? SEND_JOB_POLL_MS : false,
  });
}
