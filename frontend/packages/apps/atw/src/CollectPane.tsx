 ...
 */
export function namespaceField(field: SectionField, prefix: string): SectionField {
  if (field.type === 'one_of') {
    return {
      ...
      branches: field.branches.map((branch) => ({
        ...branch,
        fields: branch.fields.map((leaf) => ({ ...leaf, name: `${prefix}${leaf.name}` })),
      })),
    };
  }
...
export function fieldDeclaresGate(field: SectionField): boolean {
  if (field.type === 'one_of') {
    return field.branches.some((branch) => branch.fields.some(fieldDeclaresGate));
  }
...
export function buildBatchPayload(
  ...
  const overrides = (values.overrides ?? {}) as Record<string, Record<string, unknown>>;
...
export function snippetMatchesTerm(snippet: AtwSnippetSummary, term: string): boolean {
...
export function filterSnippetOptions(
  ...
  return options.filter(
    (option) => serverMatched.has(option.name) || snippetMatchesTerm(option, term),
  );
}
...
export function CollectPane({ incidentId, isClosed = false }: CollectPaneProps) {
  ...
  const searchResults =
    debouncedSearch === '' ? NO_SNIPPETS : (searchQuery.data?.items ?? NO_SNIPPETS);
  ...
  const sections = useMemo<FormSection[]>(() => {
    ...
    selected.forEach((snippet, index) => {
      const entry = merged.per_snippet.find((item) => item.snippet_filename === snippet.name);
      ...
      result.push({
        ...
        collapsible: true,
        fields: fields.map((field) => namespaceField(field, snippetPrefix(index))),
      });
    });
    return result;
      ...
      */}
      {hiddenMatchCount > 0 && (
        ...
        <Alert severity="info" role="status" sx={{ mt: 3 }}>
          Showing the first {searchResults.length} of {searchPagination?.total} snippets matching
          &ldquo;{debouncedSearch}&rdquo;. Type more of the name or description to narrow the
          results.
        </Alert>
      )}
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField
            {...params}
            label="Snippets"
            placeholder={selected.length === 0 ? 'Search or select snippets to run' : undefined}
          />
        )}
      />
