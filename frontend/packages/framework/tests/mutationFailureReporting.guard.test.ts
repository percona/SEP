...
describe('mutation failure reporting', () => {
  it('every mutation call site reports its failures in-tree', () => {
    ...
    // Assert a sentinel from each scanned area rather than a file count: the
    // count drifts with the repo and can be satisfied by the wrong tree.
    const sentinels = [
      path.join('framework', 'src', 'index.ts'),
      path.join('apps', 'atw', 'src', 'AtwApp.tsx'),
      path.join('shell', 'src', 'App.tsx'),
    ];
