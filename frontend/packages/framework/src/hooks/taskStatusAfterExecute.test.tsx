...
import { apiClient, useAppTask } from '@pmm-extensions/api';
...
function DetailPage() {
  const { data } = useAppTask<TaskRecord>(PLUGIN, TASK);
  return (
    <>
...
async function expectHeaderStatus(status: string) {
  await waitFor(() => expect(screen.getByTestId('header-status')).toHaveTextContent(status));
}
