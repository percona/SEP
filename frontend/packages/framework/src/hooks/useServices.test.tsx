...
function wrapper() {
  ...
  const client = new QueryClient({
    defaultOptions: { queries: { gcTime: 0, staleTime: 0, retry: false, retryDelay: 0 } },
  });
...
function page(items: Array<{ id: number; name: string; type: string }>, total: number, offset = 0) {
...
describe('useServices', () => {
  ...
  it('paginates until offset >= total for a single service type', async () => {
    mocked.get
      .mockResolvedValueOnce(
        page(
          Array.from({ length: 200 }, (_, i) => ({ id: i + 1, name: `s${i + 1}`, type: 'mysql' })),
          250,
        ),
      )
      .mockResolvedValueOnce(
    ...
    const { result } = renderHook(() => useServices({ serviceTypes: ['mysql'] as never[] }), {
      wrapper: wrapper(),
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  ...
  it('dedupes by id across parallel service-type fan-out', async () => {
    mocked.get.mockImplementation((_url: string, config?: { params: Record<string, unknown> }) => {
  ...
  it('surfaces error when any service-type fan-out fetch rejects', async () => {
    ...
    mocked.get.mockImplementation((_url: string, config?: { params: Record<string, unknown> }) => {
      if (config?.params.service_type === 'mysql') {
        return Promise.resolve(page([{ id: 1, name: 'svc1', type: 'mysql' }], 1));
      }
      return Promise.reject(
        new ApiError({ kind: 'http', status: 500, message: 'upstream blew up' }),
      );
    });
    ...
    expect((result.current.error as InstanceType<typeof ApiError>).status).toBe(500);
  });

  it('produces a stable query key whether serviceTypes is undefined or empty', async () => {
    mocked.get.mockResolvedValue(page([], 0));

    const { result: ra } = renderHook(() => useServices(), { wrapper: wrapper() });
    const { result: rb } = renderHook(() => useServices({ serviceTypes: [] as never[] }), {
      wrapper: wrapper(),
    });
  ...
  it('aborts pagination after MAX_PAGES iterations to prevent runaway loops', async () => {
    ...
    mocked.get.mockImplementation(() =>
      Promise.resolve(page([{ id: 1, name: 's', type: 'mysql' }], Number.MAX_SAFE_INTEGER)),
    );
  ...
  it('does not retry a deterministic 502 (sepRetry short-circuit)', async () => {
    ...
    expect(mocked.get).toHaveBeenCalledTimes(1);
    expect((result.current.error as InstanceType<typeof ApiError>).status).toBe(502);
  });

  it('respects enabled=false', async () => {
    const { result } = renderHook(() => useServices({ enabled: false }), { wrapper: wrapper() });
