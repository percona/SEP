...
describe('useSnippetDownload', () => {
  ...
  it("surfaces a refusal's own reason, not the bare status", async () => {
    ...
    const { result } = renderHook(() => useSnippetDownload(), { wrapper: QueryWrapper });
    result.current.mutate({ filename: 'check.sh' });
  ...
  it('downloads the file on success', async () => {
    ...
    const blob = new Blob(['echo hi'], { type: 'text/plain' });
    vi.spyOn(apiClient, 'get').mockResolvedValue({ data: blob });

    const { result } = renderHook(() => useSnippetDownload(), { wrapper: QueryWrapper });
    result.current.mutate({ filename: 'dir/check.sh' });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(downloadBlob).toHaveBeenCalledWith(blob, 'check.sh');
  });
});
