...
function buildEffectiveUrl(
  ...
  const path = queryStart === -1 ? endpointUrl : endpointUrl.slice(0, queryStart);
  const existingQuery = queryStart === -1 ? '' : endpointUrl.slice(queryStart + 1);
...
export function useRemoteChoices(
  ...
  const { endpointUrl, dependsOnName, dependsOnValue, enabled = true } = options;
  const effectiveUrl = buildEffectiveUrl(endpointUrl, dependsOnName, dependsOnValue);
  ...
  return useQuery<ChoiceOption[], Error>({
    ...
    enabled: enabled && (!cascades || (dependsOnValue !== null && dependsOnValue !== undefined)),
    staleTime: 60_000,
    queryFn: async () => {
