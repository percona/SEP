...
describe('useRemoteChoices', () => {
  ...
  it('does not fetch until a cascade parent value is present', async () => {
    const { result, rerender } = renderHook(
      ...
      { wrapper: wrapperFor(newClient()), initialProps: { v: null as string | null } },
    );
  ...
  it('encodes special characters in the cascade value', async () => {
    ...
    await waitFor(() => expect(mocked.get).toHaveBeenCalled());
    expect(mocked.get).toHaveBeenCalledWith('/apps/restore/backups?cluster=a+b%2Fc%26d');
  });
  ...
  it('preserves an existing query string on the endpoint URL', async () => {
    ...
    await waitFor(() => expect(mocked.get).toHaveBeenCalled());
    expect(mocked.get).toHaveBeenCalledWith('/apps/restore/backups?scope=all&cluster=c1');
  });
  ...
  it('uses distinct cache keys for the same endpoint+value under different dependsOn names', async () => {
    ...
    renderHook(
      () => useRemoteChoices({ endpointUrl: '/e', dependsOnName: 'a', dependsOnValue: 'x' }),
      {
        wrapper,
      },
    );
    renderHook(
      () => useRemoteChoices({ endpointUrl: '/e', dependsOnName: 'b', dependsOnValue: 'x' }),
      {
        wrapper,
      },
    );
