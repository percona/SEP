...
describe('TaskFilesDialog', () => {
  ...
  it('renders file rows after fetch', async () => {
    ...
    await waitFor(() => expect(screen.getByText('result.txt')).toBeInTheDocument());
  ...
  it('shows empty state when file list is empty', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/No files available/i)).toBeInTheDocument());
  });
  ...
  it('shows error alert when fetch fails', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/Failed to load file list/i)).toBeInTheDocument());
  });
  ...
  it('fires download mutation with .tar.gz name for a directory', async () => {
    ...
    const dialog = screen.getByRole('dialog');
    await userEvent.click(within(dialog).getByRole('button', { name: 'Download output/logs/' }));
  ...
  it('uses full path in aria-label to disambiguate files with the same basename', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('report.txt')).toHaveLength(2));
    expect(screen.getByRole('button', { name: 'Download stdout/report.txt' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Download stderr/report.txt' })).toBeInTheDocument();
  });

  it('disables all download buttons while a download is in flight', async () => {
    let resolveDownload!: () => void;
    mockedApiClient.get.mockResolvedValueOnce({ data: FILE_LIST }).mockImplementationOnce(
      () =>
  ...
  it('shows inline error alert on download failure', async () => {
    ...
    await waitFor(() => expect(within(dialog).getByText('Server error')).toBeInTheDocument());
  });
});
