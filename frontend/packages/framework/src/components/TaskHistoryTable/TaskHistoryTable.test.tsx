...
function mockFilesList(
  filesByHistoryId: Record<number, Record<string, { size: number; is_dir: boolean }>>,
) {
...
describe('TaskHistoryTable actions', () => {
  ...
  it('opens confirm dialog and invokes onStopTask on Stop click', async () => {
    ...
    render(
      <Wrapper client={client}>
        <TaskHistoryTable data={data} disablePolling onStopTask={onStopTask} actionError={null} />
      </Wrapper>,
    );
  ...
  it('shows download button only for completed rows with downloadable files', async () => {
    ...
    const data = [
      makeEntry(1, 'success', { task: { output_files_path: 'run-script/local/output_files' } }),
      makeEntry(2, 'success', { task: { output_files_path: 'run-script/local/output_files' } }),
      makeEntry(3, 'running', { task: { output_files_path: 'run-script/local/output_files' } }),
  ...
  it('opens built-in files dialog when no onDownloadFiles callback provided', async () => {
    ...
    const data = [
      makeEntry(5, 'success', { task: { output_files_path: 'run-script/local/output_files' } }),
    ];
    render(
      <Wrapper client={client}>
        <TaskHistoryTable data={data} disablePolling />
    ...
    await userEvent.click(await screen.findByRole('button', { name: 'Download files' }));
    const dialog = await screen.findByRole('dialog');
...
describe('TaskHistoryTable connected stop mutation', () => {
  ...
  it("reports a refused stop above the rows with the server's own reason", async () => {
    mockedApiClient.get.mockResolvedValue({
      data: { items: [makeEntry(42, 'running')], total: 1, offset: 0, limit: 10 },
    });
    ...
    expect(await screen.findByTestId('task-history-action-error')).toHaveTextContent(
    ...
    expect(() =>
      render(
        <Wrapper client={makeQueryClient()}>
          ...
          <TaskHistoryTable data={[makeEntry(2, 'running')]} disablePolling onStopTask={vi.fn()} />
        </Wrapper>,
      ),
    ).not.toThrow();
  });
