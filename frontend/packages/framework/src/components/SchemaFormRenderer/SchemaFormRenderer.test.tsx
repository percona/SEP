    ...
    const helpSections: FormSection[] = [
      {
        ...
        fields: [
          ...
          { type: 'string', name: 'notes', label: 'Notes', description: longProse },
...
describe('SchemaFormRenderer — advanced sections', () => {
  ...
  it('renders no control when the schema marks nothing advanced', () => {
    const plain = sections.map(({ advanced: _advanced, ...section }) => section);
    renderWithProviders(<SchemaFormRenderer sections={plain} onSubmit={() => {}} />);
  ...
  it('collects advanced sections wherever they appear, after the rest', async () => {
    ...
    renderWithProviders(<SchemaFormRenderer sections={interleaved} onSubmit={() => {}} />);
  ...
  it('still drops a gated-out advanced section from the payload', async () => {
    ...
    expect(onSubmit.mock.calls[0]?.[0]).not.toHaveProperty('xtrabackup_extra_args');
  });
...
describe('SchemaFormRenderer — parented fields', () => {
  ...
  it('still hides the child on a gate that is not the parent gate', async () => {
    ...
    await user.click(screen.getByLabelText('Encrypt backup'));
    await waitFor(() => expect(screen.getByLabelText('Encrypt using tmpdir')).toBeEnabled());

    await user.click(screen.getByLabelText('Encrypt after backup'));
    await waitFor(() => expect(screen.queryByLabelText('Encrypt using tmpdir')).toBeNull());
  });
  ...
  it('does not let a disabled required child block submission', async () => {
    ...
    renderWithProviders(<SchemaFormRenderer sections={requiredChild} onSubmit={onSubmit} />);
...
// ── Payload hygiene inside collapsed shells ───────────────────────────────
//
...
describe('SchemaFormRenderer — collapsed shells and the payload', () => {
  ...
  it('drops a gated field inside a never-expanded group from the payload', async () => {
    const user = userEvent.setup();
    const onSubmit = vi.fn();
    renderWithProviders(<SchemaFormRenderer sections={gatedInsideCollapsed} onSubmit={onSubmit} />);
        ...
        sections={[
          {
            ...
            fields: [
              ...
              { type: 'bool', name: 'post_run', label: 'Post run' },
              {
                ...
                label: 'Tmpdir',
                parent: 'encrypt',
                forbidden: [{ when: { falsy: 'encrypt' } }, { when: { truthy: 'post_run' } }],
              },
            ],
          },
        ]}
...
// ── Marking the odd optional field out ────────────────────────────────────
...
// ── Parent-off children show their default ────────────────────────────────
