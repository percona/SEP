 ...
 *
 * Roughly one rendered line at the form's width. The point of the boundary is
 * that a hint short enough to sit on one line costs almost nothing to leave
 * visible, while prose that wraps two or three times is what turns a form with
 * sixty fields into several screens.
 */
 ...
 */
export function isInlineHelp(field: AppField): boolean {
 ...
 */
export function fieldHelp(field: AppField): {
  tooltip?: string;
  inline?: string;
} {
  ...
  return isInlineHelp(field) ? { inline: description } : { tooltip: description };
}
