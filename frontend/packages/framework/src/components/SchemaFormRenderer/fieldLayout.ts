...
interface FieldIndex {
  ...
  byName: ReadonlyMap<string, AppField>;
}
...
const indexCache = new WeakMap<readonly AppField[], FieldIndex>();
 ...
 */
export function fieldIndex(fields: readonly AppField[]): FieldIndex {
  const cached = indexCache.get(fields);
  if (cached) {
    return cached;
  }
  const parentNames = new Set<string>();
  const byName = new Map<string, AppField>();
