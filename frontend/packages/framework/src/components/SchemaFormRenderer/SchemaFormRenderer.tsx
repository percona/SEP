...
import { useConditionalSections, useUnregisterHiddenSections } from './hooks/useConditionalSection';
 ...
 */
function isFieldDefaultValue(field: AppField, seeded: unknown): boolean {
  const base = fieldDefault(field);
  if (Array.isArray(base) && Array.isArray(seeded)) {
    return base.length === seeded.length && base.every((v, i) => Object.is(v, seeded[i]));
  }
...
const SectionRenderer = memo(function SectionRenderer({
  ...
  const [expanded, setExpanded] = useState(!section.collapsed_by_default || forceExpanded);
  ...
  return (
    <Box component="fieldset" sx={{ border: 0, p: 0, mb: section.collapsible ? 1 : 3 }}>
...
function SchemaFormBody({
  ...
  const { standardEntries, advancedEntries, afterSubmitEntries } = useMemo(() => {
    ...
    return {
      ...
      afterSubmitEntries: entries.filter(({ section }) => section.render_after_submit),
    };
  }, [sections]);
  ...
  const seededAdvanced = useMemo(
    () =>
      new Set(
        advancedEntries
          .filter(({ section }) => sectionHasNonDefaultValue(section, defaultValues))
          .map(({ index }) => index),
      ),
  ...
  const renderSection = (entry: SectionEntry, idx: number, keyPrefix: string) => (
    <SectionRenderer
  ...
  return (
    <FormFieldsProvider value={allFields}>
      ...
      <Box component="form" onSubmit={handleSubmitEvent} noValidate sx={{ maxWidth: 640 }}>
        ...
        {standardEntries.map((entry, idx) => renderSection(entry, idx, 'standard'))}
        ...
        {afterSubmitEntries.length > 0 ? (
          <Box sx={{ mt: 3 }}>
            {afterSubmitEntries.map((entry, idx) => renderSection(entry, idx, 'after'))}
          </Box>
        ) : null}
      </Box>
    </FormFieldsProvider>
