...
describe('evaluatePredicate – prototype-chain safety', () => {
  ...
  it('all_present / none_present respect own-property semantics', () => {
    ...
    expect(evaluatePredicate({ none_present: ['constructor', 'toString'] }, {})).toBe(true);
  });
  ...
  it('still honours an own-property named like an inherited member', () => {
    ...
    expect(evaluatePredicate({ truthy: 'constructor' }, { constructor: 'yes' })).toBe(true);
    expect(evaluatePredicate({ truthy: 'constructor' }, { constructor: '' })).toBe(false);
  });
});
...
describe('evaluatePredicate – $field reference to missing key', () => {
  it('equals: both sides absent collapses to true (undefined === undefined)', () => {
    ...
    expect(evaluatePredicate({ equals: { a: { $field: 'b' } } }, {})).toBe(true);
  });

  it('equals: only one side present is false', () => {
    expect(evaluatePredicate({ equals: { a: { $field: 'b' } } }, { a: 'x' })).toBe(false);
  });
...
describe('evaluatePredicate – xor arity', () => {
  ...
  it('returns true for exactly one truthy child', () => {
    expect(
      evaluatePredicate({ xor: [{ truthy: 'a' }, { truthy: 'b' }] }, { a: true, b: false }),
    ).toBe(true);
  });

  it('returns false when all children are truthy', () => {
    expect(
      evaluatePredicate({ xor: [{ truthy: 'a' }, { truthy: 'b' }] }, { a: true, b: true }),
    ).toBe(false);
  });
...
describe('evaluatePredicate – empty / unknown / nested', () => {
  ...
  it('returns false for unknown operator', () => {
    expect(evaluatePredicate({ frobnicate: 'a' } as never, { a: true })).toBe(false);
  });
  ...
  it('three-level nested not/any/all evaluates correctly', () => {
    ...
    expect(evaluatePredicate(pred, { a: false, b: false, c: true })).toBe(false);
  });
});
