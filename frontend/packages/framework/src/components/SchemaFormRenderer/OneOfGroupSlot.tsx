...
export const OneOfGroupSlot = memo(function OneOfGroupSlot({
  ...
  const { control, setValue, unregister } = useFormContext<Record<string, unknown>>();
  ...
  const activeValue = String(watchedMode ?? group.default ?? group.branches[0]?.value ?? '');

  const activeBranch = useMemo(
    () => group.branches.find((branch) => branch.value === activeValue) ?? group.branches[0],
  ...
  const handleModeChange = (_event: MouseEvent<HTMLElement>, next: string | null) => {
    ...
    setValue(group.discriminator, next, { shouldDirty: true, shouldTouch: true });
  };
