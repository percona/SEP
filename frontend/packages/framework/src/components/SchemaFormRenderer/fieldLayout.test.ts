...
describe('fieldIndex', () => {
  it('gathers every name pointed at by a `parent`', () => {
    ...
    expect([...fieldIndex(fields).parentNames].sort()).toEqual(['encrypt', 'prepare']);
  });
  ...
  it('indexes every field by name so a parent resolves to its label', () => {
    ...
    expect(fieldIndex(fields).byName.get('encrypt')?.label).toBe('Encrypt backup');
  });
  ...
  it('derives the index once per fields array', () => {
    ...
    const fields: AppField[] = [{ type: 'bool', name: 'encrypt', label: 'Encrypt' }];
  ...
  it('is empty for a schema with no parented fields', () => {
    expect(fieldIndex([{ type: 'string', name: 'a', label: 'A' }]).parentNames.size).toBe(0);
  });
});
