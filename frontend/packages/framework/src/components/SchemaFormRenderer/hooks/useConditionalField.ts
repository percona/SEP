...
import type { FieldGate, AppField, Predicate } from '../types';
...
export interface ConditionalFieldState {
   ...
   * beneath that parent, but takes no input — see {@link AppField.parent}.
   */
  isDisabled: boolean;
}
 ...
 */
export function hidingGates(field: AppField): FieldGate[] {
...
export function useConditionalField(field: AppField): ConditionalFieldState {
  ...
  useEffect(() => {
    ...
    const parentOffGate = (field.forbidden ?? []).some((g) => isParentOffGate(g.when, parent));
 ...
 */
export function useFieldPayloadCleanup(fields: AppField[]): void {
  ...
  const { hide, clear } = useMemo(() => {
    const map = watchValuesByName(watchedNames, rawValues);
    const hidden: string[] = [];
    const cleared: AppField[] = [];
    for (const field of fields) {
      if (hidingGates(field).some((gate) => evaluatePredicate(gate.when, map))) {
