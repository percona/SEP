...
describe('SchemaSelectShell', () => {
  it('renders the placeholder branch and preserves the test ids when empty', () => {
    ...
    expect(screen.getByRole('combobox').getAttribute('aria-labelledby')).toContain('fruit-label');
  });
  ...
  it('flips aria-invalid and shows the error message on error', () => {
    ...
    expect(screen.getByTestId('select-input-fruit')).toHaveAttribute('aria-invalid', 'true');
  ...
  it('keeps the info icon when an error replaces the helper text', () => {
    ...
    expect(screen.getByText('Fruit is required')).toBeInTheDocument();
    expect(screen.getByLabelText('Help for Fruit')).toBeInTheDocument();
  });
