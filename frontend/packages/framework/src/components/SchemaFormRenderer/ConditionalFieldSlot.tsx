...
export const ConditionalFieldSlot = memo(function ConditionalFieldSlot({
  ...
  const parentLabel = field.parent ? (byName.get(field.parent)?.label ?? field.parent) : undefined;
  ...
  const content = renderField?.({ field: resolvedField, renderDefault }) ?? renderDefault();
  ...
  return (
    ...
    >
      ...
      */}
      ...
      >
        {isDisabled ? `Requires ${parentLabel}, which is off` : `Part of ${parentLabel}`}
      </Box>
      {content}
    </Box>
  );
