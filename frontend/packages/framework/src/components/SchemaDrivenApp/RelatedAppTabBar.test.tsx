...
function renderTabBar(pathname: string, activeSegment?: string) {
  const segment =
    activeSegment ?? resolveRelatedAppActiveSegment(pathname, ROUTE_BASE, RELATED_APPS);
...
describe('resolveRelatedAppActiveSegment', () => {
  it('returns empty string on the parent list route', () => {
    expect(resolveRelatedAppActiveSegment('/apps/mysql_backups', ROUTE_BASE, RELATED_APPS)).toBe(
      '',
    );
  });
  ...
  it('returns the related segment on the related list route', () => {
    expect(
      resolveRelatedAppActiveSegment('/apps/mysql_backups/restores', ROUTE_BASE, RELATED_APPS),
    ).toBe('restores');
  });
