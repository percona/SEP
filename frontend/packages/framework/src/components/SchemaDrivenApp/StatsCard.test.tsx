...
vi.mock('../../hooks/useTaskStats', async () => {
  const actual = await vi.importActual<typeof import('../../hooks/useTaskStats')>(
    '../../hooks/useTaskStats',
  );
  return {
    ...actual,
...
describe('StatsCard — render states', () => {
  it('renders a skeleton while loading', () => {
    mockUseTaskStats.mockReturnValue({ data: undefined, isLoading: true, isError: false });
  ...
  it('renders inline error for 401 (does not redirect)', async () => {
    ...
    mockUseTaskStats.mockReturnValue({
      ...
      isLoading: false,
      isError: true,
      error: new ApiError({ kind: 'http', status: 401, message: 'Unauthorized' }),
    });
    renderWithClient(<StatsCard taskName="foo" />);
    expect(screen.getByRole('alert')).toHaveTextContent('Could not load execution stats');
  });
...
describe('StatsCard — defensive formatting', () => {
  ...
  it('formats absurdly large duration without crash', () => {
    mockUseTaskStats.mockReturnValue({
      data: { ...POPULATED, duration: { average_seconds: 9999999.999, last_seconds: -1 } },
      isLoading: false,
      isError: false,
    });
    renderWithClient(<StatsCard taskName="foo" />);
