...
export function AppTaskEditPage({
  ...
  const editableSections = useMemo(
    () =>
      (schema.forms ?? [])
        .map((section) => ({
          ...section,
          fields: section.fields.filter((field) => field.name !== TASK_NAME_FIELD),
        }))
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    ...
    updateTask.mutate(
      ...
      {
        onSuccess: () => {
          enqueueSnackbar(`${schema.display_name} task updated`, { variant: 'success' });
        ...
        onError: (error: unknown) => {
          ...
          setSubmitErrorState(mapSubmitError(error, editableSections, 'Failed to update'));
        },
      },
    );
  };
