...
function makePeriodic(overrides: Partial<PeriodicTaskResponse> = {}): PeriodicTaskResponse {
  return {
    ...
    next_runs: ['2026-06-18T14:00:00Z', '2026-06-18T15:00:00Z', '2026-06-18T16:00:00Z'],
    timezone: 'UTC',
    ...overrides,
  };
}
...
describe('ScheduleCell', () => {
  it('renders a muted "Not scheduled" chip when the task has no schedule', () => {
    ...
    expect(screen.getByTestId('schedule-cell-unscheduled')).toHaveTextContent('Not scheduled');
  ...
  it('renders the relative next run and a periodicity icon when scheduled', () => {
    ...
    expect(screen.getByTestId('schedule-cell-next-run')).toHaveTextContent('in 2 hours');
    expect(screen.getByTestId('schedule-cell-periodicity')).toHaveAttribute(
      'aria-label',
      'Recurs every 1 hours',
    );
    expect(screen.queryByTestId('schedule-cell-unscheduled')).not.toBeInTheDocument();
  });
  ...
  it('still renders the schedule once matched even while the list reports loading', () => {
    ...
    expect(screen.getByTestId('schedule-cell-next-run')).toHaveTextContent('in 2 hours');
    expect(screen.queryByTestId('schedule-cell-loading')).not.toBeInTheDocument();
  });
});
