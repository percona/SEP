...
export function ScheduleCell({ task, isLoading = false }: ScheduleCellProps) {
  if (!task) {
    if (isLoading) {
      return <Skeleton variant="text" width={72} data-testid="schedule-cell-loading" />;
    }
    return (
      <Chip
        label="Not scheduled"
  ...
  return (
    <Stack direction="row" spacing={0.5} alignItems="center" data-testid="schedule-cell">
      {nextRun ? (
        <Tooltip title={formatAbsoluteTime(nextRun)}>
          <Typography variant="body2" component="span" data-testid="schedule-cell-next-run">
