...
describe('useExecutionEvents', () => {
  ...
  describe('running tasks (SSE)', () => {
    ...
    it('accumulates events and groups them by step', async () => {
      ...
      act(() => {
        handle.pushMessage({ timestamp: 't1', type: 'started', description: 'A', step: 'setup' });
        handle.pushMessage({ timestamp: 't2', type: 'progress', description: 'B', step: 'setup' });
        handle.pushMessage({ timestamp: 't3', type: 'started', description: 'C', step: 'test' });
      });
    ...
    it('buckets events without a step under the stepless key', async () => {
      ...
      act(() => {
        handle.pushMessage({ timestamp: 't1', type: 'started', description: 'A' });
        handle.pushMessage({ timestamp: 't2', type: 'started', description: 'B', step: null });
      });
  ...
  describe('transition', () => {
    it('switches from SSE to REST when isRunning flips to false', async () => {
      mockApiGet.mockResolvedValue({
        data: [{ timestamp: 't-final', type: 'completed', description: 'done', step: 'x' }],
      } as never);
