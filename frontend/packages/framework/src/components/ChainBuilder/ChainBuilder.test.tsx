...
const TASKS = [{ name: 'task-a' }, { name: 'task-b' }, { name: 'task-c' }, { name: 'task-self' }];
...
describe('ChainBuilder rendering', () => {
  it('renders no chip sequence and no failure toggle when chain is empty', () => {
    ...
    expect(screen.queryByTestId('chain-on-failure-checkbox')).not.toBeInTheDocument();
    expect(screen.getByRole('combobox')).toBeInTheDocument();
  });

  it('exposes the widget as a labelled group', () => {
    render(<Harness />);
    const group = screen.getByRole('group', { name: /chain tasks after execution/i });
    expect(group).toBeInTheDocument();
  });

  it('renders chips and failure toggle when chain is non-empty', () => {
    render(
      <Harness initial={{ chain_task_names: ['task-a', 'task-b'], chain_on_failure: false }} />,
    );
...
describe('ChainBuilder add', () => {
  it('appends the selected task and emits the new chain', async () => {
    ...
    expect(within(screen.getByTestId('chain-sequence')).getByText('task-a')).toBeInTheDocument();
  });
  ...
  it('disables already-chained tasks in the dropdown', async () => {
    const user = userEvent.setup();
    render(<Harness initial={{ chain_task_names: ['task-a'], chain_on_failure: false }} />);
    await user.click(screen.getByRole('combobox'));
  ...
  it('disables the dropdown entirely when no selectable tasks remain', () => {
    render(<Harness availableTasks={[{ name: 'task-self' }]} currentTaskName="task-self" />);
    expect(screen.getByRole('combobox')).toHaveAttribute('aria-disabled', 'true');
    expect(screen.getByText(/no tasks available to chain/i)).toBeInTheDocument();
  });
});
...
describe('ChainBuilder remove', () => {
  it('removes a task when its remove button is clicked', async () => {
    ...
    render(
      <Harness
        initial={{ chain_task_names: ['task-a', 'task-b'], chain_on_failure: true }}
  ...
  it('removes only the clicked occurrence when names duplicate (stale data)', async () => {
    ...
    render(
      <Harness
        initial={{ chain_task_names: ['task-a', 'task-b', 'task-a'], chain_on_failure: false }}
        onChangeSpy={onChange}
      />,
    );

    // Two chips share the same accessible name "Remove task-a" — click the first.
    const removeButtons = screen.getAllByRole('button', { name: 'Remove task-a' });
...
describe('ChainBuilder chain_on_failure toggle', () => {
  ...
  it('hides the toggle when the chain becomes empty after a remove', async () => {
    const user = userEvent.setup();
    render(<Harness initial={{ chain_task_names: ['task-a'], chain_on_failure: true }} />);
    expect(screen.getByTestId('chain-on-failure-checkbox')).toBeInTheDocument();
...
describe('ChainBuilder disabled', () => {
  it('disables drag, remove, dropdown, and failure toggle when disabled', () => {
    render(
      <Harness disabled initial={{ chain_task_names: ['task-a'], chain_on_failure: false }} />,
    );
    expect(screen.getByRole('button', { name: 'Drag task-a' })).toBeDisabled();
    expect(screen.getByRole('button', { name: 'Remove task-a' })).toBeDisabled();
    expect(screen.getByRole('combobox')).toHaveAttribute('aria-disabled', 'true');
