...
const FAILURE_TITLE = 'When enabled, the chain continues even if a task fails, stops, or is lost';
...
export function ChainBuilder({
  ...
  const itemIds = useMemo(() => chain.map((name, i) => chipId(i, name)), [chain]);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    onChange({ ...value, chain_task_names: arrayMove(chain.slice(), from, to) });
  };
  ...
  const selectHelper = !hasSelectableTask ? 'No tasks available to chain' : undefined;

  return (
    <Box role="group" aria-labelledby={groupLabelId} data-testid="chain-builder">
      <Typography
        id={groupLabelId}
        component="div"
        variant="body2"
      ...
      <FormControl fullWidth size="small" disabled={selectDisabled}>
        ...
        {selectHelper && (
          <Typography id={helperId} variant="caption" color="text.secondary" sx={{ mt: 0.5 }}>
            {selectHelper}
          </Typography>
        )}
      </FormControl>
