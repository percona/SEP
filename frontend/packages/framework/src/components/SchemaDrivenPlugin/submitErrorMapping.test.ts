...
function http(status: number, detail?: unknown): ApiError {
  return new ApiError({ kind: 'http', status, message: `HTTP ${status}`, data: { detail } });
}

describe('mapSubmitError', () => {
  it('returns the empty state for non-422 errors (toast-only path unchanged)', () => {
    expect(mapSubmitError(http(500), SECTIONS, 'Failed')).toBe(EMPTY_SUBMIT_ERROR);
    expect(
      mapSubmitError(new ApiError({ kind: 'network', message: 'x' }), SECTIONS, 'Failed'),
    ).toBe(EMPTY_SUBMIT_ERROR);
    expect(mapSubmitError(new Error('boom'), SECTIONS, 'Failed')).toBe(EMPTY_SUBMIT_ERROR);
  });
  ...
  it('surfaces an unmatched/unknown field error in the banner using its raw path', () => {
    ...
    expect(result.fieldErrors).toEqual([{ path: 'mystery', message: 'not allowed here' }]);
    expect(result.submitError).toBe(['Failed', '• mystery: not allowed here'].join('\n'));
  });
