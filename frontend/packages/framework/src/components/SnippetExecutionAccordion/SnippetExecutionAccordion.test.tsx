...
describe('SnippetExecutionAccordion', () => {
  ...
  it('uses filename as fallback title', () => {
    renderWithProviders(
      <SnippetExecutionAccordion snippetFilename="check.sh" executorHost="db1" />,
    );
  ...
  it("reports a failed stop above the history with the server's own reason", async () => {
    ...
    mockedApi.post.mockRejectedValue(new Error("You don't have permission to perform this action"));
