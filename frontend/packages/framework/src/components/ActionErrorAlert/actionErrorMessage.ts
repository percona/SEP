 ...
 * their own (a batch endpoint listing per-item failures), and those already have
 * a usable ``ApiError.message``.
 *
 * ``fallback`` is reached only when neither path yields text — an ``Error``
 * with an empty message, or a thrown non-error value.
 */
