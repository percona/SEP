...
describe('actionErrorMessage', () => {
  it("uses the refusal's own reason for a 403", () => {
    expect(actionErrorMessage(http(403, "You don't have permission to perform this action"))).toBe(
  ...
  it('ignores an array detail on any status other than 422', () => {
    ...
    expect(actionErrorMessage(error)).toBe('Batch approval failed for 2 snippets');
  });

  it('reports transport failures with their own message', () => {
    expect(actionErrorMessage(new ApiError({ kind: 'network', message: 'Network error' }))).toBe(
      'Network error',
    );
    expect(
      actionErrorMessage(new ApiError({ kind: 'timeout', message: 'Request timed out' })),
    ).toBe('Request timed out');
  });

  it('falls back only when nothing carries a message', () => {
    expect(actionErrorMessage(new Error(''))).toBe(DEFAULT_ACTION_ERROR_FALLBACK);
...
describe('ActionErrorAlert', () => {
  ...
  it("renders the server's reason", () => {
    ...
    expect(screen.getByTestId('action-error-alert')).toHaveTextContent('Sync already running');
  });
