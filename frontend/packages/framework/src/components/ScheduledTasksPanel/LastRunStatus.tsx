 ...
 *
 * The backend leaves `last_run_status` null in two distinct situations
 * (`app/tasks/periodic/utils.py`): the schedule has genuinely never run
 * (`last_run_at` is also null), or it ran but no matching execution-history
 * point could be resolved (`last_run_at` is set). These are disambiguated with
 * `lastRunAt` so a task that has run is never mislabeled "Not yet run" next to
 * a real timestamp. A present-but-unrecognized status is shown verbatim rather
 * than dropped, matching the fallback used by the list/detail status columns.
 */
