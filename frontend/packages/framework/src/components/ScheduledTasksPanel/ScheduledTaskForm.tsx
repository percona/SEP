...
export function ScheduledTaskForm({
  ...
  useEffect(() => {
    ...
    if (chain.chain_task_names.includes(taskName)) {
      setValue(
        ...
        {
          ...chain,
          chain_task_names: chain.chain_task_names.filter((n) => n !== taskName),
        },
        { shouldDirty: true },
      );
    }
  ...
  const toggleMode = () => {
    setValue('scheduleMode', scheduleMode === 'interval' ? 'cron' : 'interval', {
      shouldDirty: true,
    });
  };
  ...
  return (
    ...
    >
      ...
      <Stack direction="row" spacing={2} flexWrap="wrap" alignItems="flex-start" sx={{ mb: 2 }}>
        {taskField}
        ...
        {scheduleMode === 'interval' ? (
          <Stack direction="row" spacing={1} alignItems="center">
            <TextField
              ...
              label="Every"
              required
              slotProps={{ htmlInput: { min: 1, 'data-testid': 'sched-form-interval-every' } }}
            ...
            <Stack direction="row" spacing={1}>
              <TextField
                ...
                {...register('cronExpression', {
                  required: true,
                  validate: (v) => humanize(v).valid || 'Invalid cron expression',
                })}
      ...
      <Box sx={{ mb: 1 }}>
        ...
        >
          {scheduleMode === 'interval' ? 'change to cron mode' : 'change to interval mode'}
        </Link>
      </Box>
