...
describe('toDisplayValue', () => {
  it('resolves a stored value string to its option', () => {
    expect(toDisplayValue('backup-1', OPTIONS)).toEqual({ value: 'backup-1', label: 'Backup 1' });
  });
  ...
  it('resolves a stored option object (back-compat) to the matching option', () => {
    expect(toDisplayValue({ value: 'backup-2', label: 'Backup 2' }, OPTIONS)).toEqual(OPTIONS[1]);
  });
