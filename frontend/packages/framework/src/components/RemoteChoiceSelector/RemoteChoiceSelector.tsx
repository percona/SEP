...
const PARENT_MISSING_CUSTOM_TEXT = 'Select a value first to list options, or type one';
...
function RemoteChoiceAutocomplete({
  ...
  return (
    <Autocomplete<ChoiceOption | string, false, false, boolean>
      ...
      getOptionDisabled={(option) => typeof option !== 'string' && Boolean(option.disabled)}
      ...
      filterOptions={
        allowCustom
          ? (opts, params) => {
              ...
              const exists = options.some((o) => o.label === input || o.value === input);
              if (input !== '' && !exists) {
                filtered.push(input);
              }
              return filtered;
...
export function RemoteChoiceSelector({
  ...
  const parentRaw = useWatch({ control, name: dependsOn ?? '', disabled: !dependsOn });
  ...
  const empty = !parentMissing && !isLoading && !isError && options.length === 0;
  const parentMissingText = allowCustomEnabled ? PARENT_MISSING_CUSTOM_TEXT : PARENT_MISSING_TEXT;
    ...
    <Controller
      ...
      render={({ field, fieldState: { error: fieldError } }) => (
        <RemoteChoiceAutocomplete
          ...
          disabled={disabled || (!allowCustomEnabled && (parentMissing || isError))}
