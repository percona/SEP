...
import { normalizeChange, toDisplayValue, type ReferenceOption } from './freeSoloValue';
...
describe('normalizeChange', () => {
  it('commits the id when an option object is picked', () => {
    expect(normalizeChange({ id: 10, name: 'app_prod' }, OPTIONS, labelOf)).toBe(10);
  });

  it('commits a string host id when a host option is picked', () => {
    const hosts: ReferenceOption[] = [{ id: 'nomad-1', name: 'db-mysql-prod-01' }];
