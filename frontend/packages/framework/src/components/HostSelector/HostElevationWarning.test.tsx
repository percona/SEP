...
function Harness({ host, snippets }: { host: unknown; snippets: SnippetElevation[] }) {
...
describe('HostElevationWarning', () => {
  ...
  it('reads a host held as a bare executor id, as a free-solo field stores it', async () => {
    ...
    expect(await screen.findByText(/db-01 cannot run commands with sudo/)).toHaveTextContent(
      'always.sh',
    );
  });
