...
/** Minimal service shape needed to resolve an executor host (Dipper order). */
 ...
 *
 * Mirrors ``resolve_executor_host_for_service`` in the Dipper app:
...
export function resolveExecutorHostForService(
  ...
  if (service.node?.address) {
    const byAddress = hosts.find((host) => host.address === service.node!.address);
    if (byAddress) {
      return byAddress;
    }
  }
