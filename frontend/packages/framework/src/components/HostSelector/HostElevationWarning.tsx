...
export function snippetsLaunchedWithSudo<T extends SnippetElevation>(
  ...
  return snippets.filter(
    (snippet) => snippet.sudo === 'always' || (sudoChosen && snippet.sudo === 'optional'),
  );
}
 ...
 */
export function HostElevationWarning({ name, sudoName, snippets }: HostElevationWarningProps) {
  ...
  const sudoIsOptionalForAll = unlaunchable.every((snippet) => snippet.sudo === 'optional');

  return (
    <Alert severity="warning" sx={{ mt: 1 }}>
      {host.name} cannot run commands with sudo, so these snippets will fail to launch on it:{' '}
      {unlaunchable.map((snippet) => snippet.title).join(', ')}.{' '}
      {sudoIsOptionalForAll ? 'Turn sudo off, or choose another host.' : 'Choose another host.'}
    </Alert>
  );
}
