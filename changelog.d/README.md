...
**Pick the section from the work-item type — between `fixed` and `changed`.**
File a Bug's fragment under `fixed` and a Story's under `changed`. The type
decides even when two tickets do topically identical work.

That rule scopes to the `fixed` / `changed` pair, which is the confusable one:
both describe a modification to behaviour that already shipped, so without a
tiebreak two tickets doing identical work land in different sections. It does
**not** reach the other sections, and reading it as exhaustive makes `added`
unreachable for the Story that ships a new surface — the exact case `added`
exists for.

Pick by what the change *is* first, and only fall back to the type rule when the
answer is genuinely "a modification to existing behaviour":
...
| `fixed` / `changed` | everything else — and here the type decides: Bug → `fixed`, Story → `changed`. |
