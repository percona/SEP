...
def _reset_tasks_settings():
    """Restore the cap/eviction-row settings after each test so caps do not leak."""
    original_cap = tasks_settings.LOG_STREAM_CAP_BYTES
    original_max_rows = tasks_settings.LOG_STREAM_EVICTION_MAX_ROWS
    yield
    tasks_settings.LOG_STREAM_CAP_BYTES = original_cap
    tasks_settings.LOG_STREAM_EVICTION_MAX_ROWS = original_max_rows


def _set_cap(cap_bytes: int, max_rows: int = 1000) -> None:
    """Override the cap/eviction-row settings on the tasks settings proxy."""
    tasks_settings.LOG_STREAM_CAP_BYTES = cap_bytes
    tasks_settings.LOG_STREAM_EVICTION_MAX_ROWS = max_rows
