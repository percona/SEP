...
``app/extensions/apps/``, so a module that ships in the image must hold no import-time
...
``app/extensions/apps/`` plus, inside it, ``framework``/``shared`` and the apps-level
...
(``import_module("app.extensions.apps.alerts.config")``,
``import_var("app.extensions.apps.alerts.config:alerts_settings")``).
...
import-time rule cannot see: nothing under ``app/extensions/apps/`` may import the
...
FORM_BACKFILL_ORCHESTRATOR = "app.extensions.apps.framework.form_backfill"

FORM_BACKFILL_REGISTRY = "app.extensions.apps.framework.form_backfill_registry"


def _app_package_names(apps_root: Path) -> set[str]:
    """Return every activatable app package directory under ``app/extensions/apps/``.
...
def _direct_import_edges(node: ast.AST, package: str) -> Iterator[tuple[str, int]]:
    ...
    ``from app.extensions.apps import alerts`` names the loaded package in the alias
...
def _guarded_module_paths() -> Iterator[Path]:
    ...
    ``app/extensions/apps/__init__.py``, and any name-based skip leaves a real module
...
def _apps_tree_module_paths() -> Iterator[Path]:
    """Yield every ``app/extensions/apps/**/*.py`` module the apps-tree guards scan.

    :return: An iterator of source paths under ``app/extensions/apps/``.
    """
...
def _parse_module(path: Path) -> ast.Module | None:
    ...
    ``app/extensions/apps/`` while this walk is listing and reading it. A path listed a moment
...
def _app_package_of(module: str, app_packages: set[str]) -> str | None:
    ...
    """
    parts = module.split(".")
    if parts[:3] != ["app", "extensions", "apps"] or not parts[3:4]:
        return None
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "app/extensions/main.py",
            "from app.extensions.apps.alerts.config import AlertsSettings\n",
            ["app/extensions/main.py:1 -> app.extensions.apps.alerts.config"],
            id="import-time-edge-reported-once",
        ),
        pytest.param(
            "app/extensions/apps/inventory/deps.py",
            "from app.extensions.apps.inventory.sync import run_inventory_sync\n",
            [],
            id="own-package-edge-exempt",
        ),
        pytest.param(
            "app/extensions/main.py",
            "from app.extensions.apps import alerts, dipper\n",
            [
                "app/extensions/main.py:1 -> app.extensions.apps.alerts",
                "app/extensions/main.py:1 -> app.extensions.apps.dipper",
            ],
...
def _deferred_violations(
    ...
    collapse a statement reaching *two* app packages (``from app.extensions.apps
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "app/extensions/main.py",
            "def _lazy():\n    from app.extensions.apps.alerts.config import AlertsSettings\n",
            ["app/extensions/main.py:2 -> app.extensions.apps.alerts.config"],
        ...
        pytest.param(
            ...
            "def _lazy():\n    from app.extensions.apps import alerts, dipper\n",
            [
                "app/extensions/main.py:2 -> app.extensions.apps.alerts",
                "app/extensions/main.py:2 -> app.extensions.apps.dipper",
            ],
            id="two-packages-on-one-line-both-reported",
        ),
    ],
...
def test_parse_module_parses_a_module_that_exists(tmp_path: Path) -> None:
    ...
    assert [target for target, _ in _declared_imports(tree, "app.extensions")] == [
        "app.extensions.apps.alerts.config",
        "app.extensions.apps.alerts.config.alerts_settings",
    ]
...
def test_parsed_modules_walks_past_a_path_that_vanished() -> None:
    ...
    """
    present = BASE_DIR / "app" / "extensions" / "main.py"
...
def _apps_tree_modules_not_guarded(guarded: set[Path] | None = None) -> set[Path]:
    ...
    :return: Paths under ``app/extensions/apps/`` the guarded walk does not cover.
    """
...
@pytest.mark.parametrize(
    ...
    [
        "app/extensions/main.py",
        "app/extensions/apps/framework/form_backfill.py",
        "app/extensions/apps/shared/disk_script_source.py",
        "app/extensions/apps/__init__.py",
        "app/extensions/apps/labels.py",
        "app/extensions/apps/nav_icons.py",
        "app/extensions/apps/inventory/app.py",
        "app/extensions/apps/alters/app.py",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "app/extensions/apps/inventory/deps.py",
            "from app.extensions.apps.alters.app import app",
            {"alters"},
            id="sibling-package-not-exempt",
        ),
        pytest.param(
            "app/extensions/apps/mysql_backups/restore/app.py",
            "from app.extensions.apps.mysql_backups.restore.app import app",
            set(),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "from app.extensions.apps import alerts",
            {"alerts"},
        ...
        pytest.param(
            "import app.extensions.apps.alerts.config",
            {"alerts"},
            id="plain-import",
        ),
        pytest.param(
        ...
        pytest.param(
            'import_var(path="app.extensions.apps.alerts.config:alerts_settings")',
            {"alerts"},
            id="dynamic-keyword-target",
        ),
        pytest.param(
            "from app.extensions.apps import labels, nav_icons",
            set(),
            id="shared-modules",
        ),
        pytest.param(
...
def test_import_time_edges_resolve_only_stripped_app_packages(
    ...
    Each source is parsed as if it were a module in ``app.extensions``, so the relative
    cases resolve against the package a core module like ``app/extensions/main.py``
    would carry.
    """
    app_packages = _app_package_names(APPS_ROOT)
    reached = {
        package
        for module, _ in _import_time_imports(ast.parse(source), "app.extensions")
...
def test_shared_app_modules_are_not_treated_as_app_packages(name: str) -> None:
    """Omit the shared ``app/extensions/apps/*.py`` modules from the app package set."""
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "app.extensions.apps.framework.form_backfill",
            FORM_BACKFILL_ORCHESTRATOR,
            id="exact-orchestrator",
        ),
        pytest.param(
            "app.extensions.apps.framework.form_backfill.FormBackfillContext",
            FORM_BACKFILL_ORCHESTRATOR,
            id="orchestrator-alias",
        ),
        pytest.param(
            "app.extensions.apps.framework.form_backfill_registry.FormBackfillEntry",
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "app.extensions.apps.framework.form_backfill_registry",
            id="registry",
        ),
        pytest.param(
            "app.extensions.apps.framework.form_backfill_inventory",
            id="inventory",
        ),
    ],
)
...
def test_declared_imports_count_type_checking_guards() -> None:
    ...
    source = (
        "if TYPE_CHECKING:\n"
        "    from app.extensions.apps.framework.form_backfill import FormBackfillContext\n"
    )
    tree = ast.parse(source)
    declared = {module for module, _ in _declared_imports(tree, "app.extensions")}
    import_time = {module for module, _ in _import_time_imports(tree, "app.extensions")}
...
def _orchestrator_import_edges(
    ...
    :param modules: The ``(path, tree)`` pairs to scan. Defaults to the whole
        ``app/extensions/apps/`` tree parsed from disk, skipping any path that vanished
...
def test_no_apps_module_imports_the_form_backfill_orchestrator() -> None:
    """Reject any import of the orchestrator from under ``app/extensions/apps/``.
    ...
    assert not found, (
        "no module under app/extensions/apps/ may import"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "app/extensions/apps/alerts/app.py",
            "from app.extensions.apps.framework.form_backfill import FormBackfillContext\n",
            [
                "app/extensions/apps/alerts/app.py:1 -> app.extensions.apps.framework.form_backfill"
            ],
        ...
        pytest.param(
            ...
            "    from app.extensions.apps.framework.form_backfill import FormBackfillContext\n",
            [
                "app/extensions/apps/alerts/app.py:2 -> app.extensions.apps.framework.form_backfill"
            ],
            id="type-checking-edge-reported",
        ),
        pytest.param(
        ...
        pytest.param(
            "app/extensions/apps/alerts/app.py",
            "from app.extensions.apps.framework.form_backfill_registry import"
            " FormBackfillEntry\n",
            [],
