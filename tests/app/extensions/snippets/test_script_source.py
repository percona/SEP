...
session (the same pattern ``tests/app/extensions/apps/snippets/test_celery.py`` uses for
...
from app.extensions.apps.framework.script_source import ScriptExecuteWrite
...
from app.extensions.snippets.deps import build_snippet_execution_meta
from app.extensions.snippets.list_query import SnippetApprovalFilter, SnippetListQuery
...
def _snippet_query(
    ...
    Deliberately narrower than its ``tests/app/extensions/snippets/test_crud.py`` namesake:
...
class TestBuildSnippetSource:
    ...
    async def test_raises_400_when_no_base_url(
        ...
        monkeypatch.setattr(
            "app.extensions.apps.framework.script_helpers.settings.BASE_URL", None
        )
        await create_snippet("ok.sh", approved=True)
        script = await snippet_source.load_script("ok.sh")
        with pytest.raises(HTTPBadRequestException):
