...
import app.extensions.snippets.models as snippets_models
...
class TestModelsPackageImportBoundary:
    ...
    ``app/extensions/migrations/env.py`` star-imports this package and ``crud.py``
    imports it, so a root-level re-export would drag
    ``app.extensions.apps.framework.schema`` -- and through it ``app.inventory.models``
