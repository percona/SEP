...
"""Define tests for the app.extensions.snippets.celery module."""
...
import app.extensions.snippets.celery as extensions_celery
from app.extensions.snippets.celery import (
...
MODULE = "app.extensions.snippets.celery"
...
class TestUpdateSnippets:
    ...
    async def test_creates_new_snippet(self, session, mocker, tmp_path):
        ...
        await extensions_celery.update_snippets()
...
class TestBuiltinAutoApproval:
    ...
    async def test_absent_from_manifest_skips_hashing(self, session, mocker, tmp_path):
        ...
        (tmp_path / "custom.sh").write_bytes(b"#!/bin/bash\necho custom\n")
        spy = mocker.spy(extensions_celery, "sha256_file")
    ...
    async def test_toggle_disabled_skips_auto_approval(self, session, mocker, tmp_path):
        ...
        content = b"#!/bin/bash\necho builtin\n"
        _write_manifest(tmp_path, {"builtin.sh": content})
        load_manifest = mocker.spy(extensions_celery, "load_builtin_checksum_manifest")
...
class TestUpdateSnippetsIsDrainUnowned:
    ...
    def test_sync_snippets_carries_no_owner_tag(self):
        ...
        assert getattr(extensions_celery.sync_snippets, "owner_app_key", None) is None

    def test_module_imports_no_drain_helpers(self):
        """Assert the cooperative-cancel seam is gone, not merely unused."""
        assert not hasattr(extensions_celery, "should_cancel")
        assert not hasattr(extensions_celery, "owned_by")
    ...
    async def test_completes_full_sync_without_consulting_drain_state(
        ...
        should_cancel = mocker.patch(
            "app.extensions.app_drain.should_cancel", new=AsyncMock(return_value=True)
        )
