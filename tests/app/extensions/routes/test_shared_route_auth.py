...
from app.extensions.deps import get_tasks_api, get_tasks_client
...
def other_user_task_history(test_client, task_history_response):
    ...
    previous = dict(extensions_app.dependency_overrides)
    extensions_app.dependency_overrides[get_tasks_api] = lambda: tasks_api
    extensions_app.dependency_overrides[get_tasks_client] = lambda: tasks_client
    try:
        yield task_history_response
    finally:
        extensions_app.dependency_overrides.clear()
        extensions_app.dependency_overrides.update(previous)
