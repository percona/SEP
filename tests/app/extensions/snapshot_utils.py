...
UPDATE = os.environ.get("EXTENSIONS_UPDATE_SNAPSHOTS") not in (
    None,
    "",
    "0",
    "false",
...
def configured_plugin_keys() -> list[str]:
    ...
    ``extensions_settings.APPS`` and never mutated) rather than ``extensions_settings``
...
def build_plugins_openapi() -> dict[str, Any]:
    ...
    than reading the process-global ``extensions_app``. Sibling conftests mutate
    ``extensions_app`` at import time (``backup_pg`` injects routers), which both
    freezes ``extensions_app``'s cached schema for other tests and perturbs shared
    ``components/schemas`` names — so a snapshot taken from it would depend on
    test-suite composition. ``api_router`` is built once from
    ``extensions_settings.APPS`` and never mutated, so its schema is deterministic.
    ``generate_tag_prefixed_unique_id`` matches the ``operationId`` scheme that
    ``create_app`` installs on ``extensions_app``.
...
def assert_or_update(golden: Path, content: str) -> None:
    ...
    In update mode (``EXTENSIONS_UPDATE_SNAPSHOTS`` set to a truthy value) write
...
def assert_golden_set_matches(subdir: str, expected_slugs: set[str]) -> None:
    ...
    if on_disk != expected_slugs:
        raise AssertionError(
            ...
            f"regenerate with EXTENSIONS_UPDATE_SNAPSHOTS=1"
        )
