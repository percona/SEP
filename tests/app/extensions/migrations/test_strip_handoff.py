...
from app.extensions.migrations._orphan_heads import empty_version_locations
...
_ALPHA_ENTRY = "%(here)s/app/extensions/apps/alpha/migrations/versions"
...
def _build_handoff_tree(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    """
    apps_root = tmp_path / "app" / "extensions" / "apps"
    main_versions = tmp_path / "app" / "extensions" / "migrations" / "versions"
    alpha = apps_root / "alpha"
    alpha_versions = alpha / "migrations" / "versions"

    (tmp_path / "app" / "extensions" / "migrations").mkdir(parents=True)
    write_revision(main_versions, "main00000001", "extensions_main")
...
def _filter_armed(ini_path: Path) -> bool:
    ...
    script = ScriptDirectory.from_config(
        Config(str(ini_path), ini_section="extensions")
    )
