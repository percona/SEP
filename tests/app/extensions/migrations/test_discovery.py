...
"""Define tests for PMM Extensions migration discovery helpers."""
...
import app.extensions.apps as plugins_pkg
from app.extensions.config import extensions_settings
from app.extensions.migrations._discovery import (
    _load_models_module,
...
def isolated_plugins_path(tmp_path, monkeypatch):
    ...
    Replace ``app.extensions.apps.__path__`` with a single-entry list
...
def test_discover_ignores_extensions_settings_plugins(monkeypatch):
    """Assert discovery does not consult ``extensions_settings.APPS``."""
    monkeypatch.setattr(extensions_settings, "APPS", [])
    version_dirs = discover_plugin_migrations_and_models()
    alerts_versions = str(
        Path(plugins_pkg.__path__[0]) / "alerts" / "migrations" / "versions"
    )
...
def test_discover_skips_plugin_without_migrations_dir(isolated_plugins_path):
    ...
    version_dirs = discover_plugin_migrations_and_models()
    assert version_dirs == []
    assert "app.extensions.apps.no_migrations_plugin.models" not in sys.modules
...
def test_discover_propagates_broken_models_import(isolated_plugins_path):
    ...
    with pytest.raises(ModuleNotFoundError):
        discover_plugin_migrations_and_models()
    assert "app.extensions.apps.broken_models_plugin.models" not in sys.modules
...
def test_discover_does_not_trigger_plugin_init_py(isolated_plugins_path):
    ...
    loaded = sys.modules["app.extensions.apps.init_raises_plugin.models"]
...
def test_discover_plugin_version_dirs_sorted_without_loading_models(tmp_path):
    ...
    assert "app.extensions.apps.alpha.models" not in sys.modules
    assert "app.extensions.apps.zebra.models" not in sys.modules
