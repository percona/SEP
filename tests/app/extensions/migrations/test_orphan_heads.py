...
"""Define tests for the PMM Extensions orphan-head filtering helpers.
...
from app.extensions.migrations._orphan_heads import (
...
def test_partition_heads_splits_known_from_unknown(extensions_script):
    """Sort a resolvable revision and an unknown one into separate tuples."""
    resolvable, unresolvable = partition_heads(
        extensions_script, (ALERTS_HEAD, UNKNOWN_REVISION)
    )
...
def test_partition_heads_treats_every_real_head_as_resolvable(extensions_script):
    """Leave ``unresolvable`` empty for the heads the tree actually defines."""
    heads = tuple(rev.revision for rev in extensions_script.get_revisions("heads"))

    resolvable, unresolvable = partition_heads(extensions_script, heads)
...
def test_partition_heads_preserves_input_order(extensions_script):
    """Keep resolvable ids in the order they appeared in ``heads``."""
    resolvable, unresolvable = partition_heads(
        extensions_script, (_ATW_REVISION, UNKNOWN_REVISION, _MYSQL_BACKUPS_HEAD)
    )
...
def test_partition_heads_reports_several_unknown_ids(extensions_script):
    """Collect every unresolvable id, as an app with unmerged heads produces."""
    _, unresolvable = partition_heads(
        extensions_script, (UNKNOWN_REVISION, "cafebabe5678", ALERTS_HEAD)
    )

    assert unresolvable == (UNKNOWN_REVISION, "cafebabe5678")


def test_empty_version_locations_is_empty_when_every_entry_is_populated(
    extensions_script,
):
    """Report nothing empty for the checked-in configuration."""
    assert empty_version_locations(extensions_script) == (), (
        "A configured [extensions] version_locations entry contributes no revisions, so "
...
class TestFailClosedDiagnostic:
    ...
    @staticmethod
    def _read_fail_closed_error(extensions_script, caplog) -> str:
        """Run the filter over an unresolvable head and return the ERROR text.

        :param extensions_script: Script directory whose locations are all populated.
        ...
        env_context = SimpleNamespace(
            get_context=lambda: migration_context, script=extensions_script
        )
    ...
    def test_offers_each_candidate_cause(self, extensions_script, caplog, cause):
        """Name every explanation an operator has to rule out by hand."""
        assert cause in self._read_fail_closed_error(extensions_script, caplog)

    def test_does_not_rule_out_a_stripped_app(self, extensions_script, caplog):
        """Leave a stripped app in play: a pruned entry hides the evidence."""
        message = self._read_fail_closed_error(extensions_script, caplog)

        assert message
        assert "not a stripped app" not in message

    def test_names_what_the_operator_has_to_look_at(self, extensions_script, caplog):
        """Point at the offending revision and the table recording it."""
        message = self._read_fail_closed_error(extensions_script, caplog)

        assert UNKNOWN_REVISION in message
        assert "alembic_version_extensions" in message

    def test_requires_every_location_to_contribute_a_revision(
        self, extensions_script, caplog
    ):
