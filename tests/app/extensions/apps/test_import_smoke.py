...
(``from app.extensions.apps.<app>.routes import router``). Deleting ``routes.py``
...
import app.extensions.apps as apps_package
...
def test_app_package_imports(package_name: str) -> None:
    ...
    assert importlib.import_module(f"app.extensions.apps.{package_name}") is not None
...
def test_app_package_exports_no_jinja_router(package_name: str) -> None:
    ...
    package = importlib.import_module(f"app.extensions.apps.{package_name}")
