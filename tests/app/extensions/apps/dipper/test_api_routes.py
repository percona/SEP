...
from app.extensions.apps.dipper.deps import get_pmm_api
...
class TestDipperFormSchemaEndpoint:
    ...
    def test_pmm_schema_falls_back_to_text_when_pmm_unconfigured(
        ...
        mock_task_api_dep.get = AsyncMock(return_value={})
        sentinel = object()
        previous = extensions_app.dependency_overrides.get(get_pmm_api, sentinel)
        ...
        try:
            ...
            if previous is sentinel:
                extensions_app.dependency_overrides.pop(get_pmm_api, None)
            else:
                extensions_app.dependency_overrides[get_pmm_api] = previous
...
class TestDipperExecuteEndpoint:
    ...
    def test_execute_returns_422_when_args_fail_schema_validation(
        ...
        with patch(
            "app.extensions.apps.dipper.deps.DipperScript.from_path",
    ...
    def test_execute_pmm_returns_422_when_no_pmmserver_configured(
        ...
        with patch("app.extensions.apps.dipper.deps.settings") as mock_settings:
