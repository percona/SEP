...
from app.extensions.api.routes.apps import build_navigation_react_route
...
async def override_session_fixture() -> AsyncIterator[AsyncSession]:
    """Provide an in-memory SQLite PMM Extensions session pre-loaded with all tables."""
...
def api_user_client_fixture(
    ...
    """Yield an authenticated (non-admin) client with the in-memory PMM Extensions session."""
...
class TestListAppsForNavigation:
    ...
    async def test_dependency_disabled_app_names_its_blocker(
        ...
        monkeypatch.setattr(
            "app.extensions.api.routes.apps.get_app_registry", lambda: registry
        )
        override_session.add(
