...
from app.sep.plugins.snippets.deps import build_snippet_execution_meta
from app.sep.plugins.snippets.script_source import (
...
class TestBuildSnippetSource:
    ...
    async def test_raises_400_when_no_base_url(
        ...
        monkeypatch.setattr(
            "app.sep.plugins.snippets.script_source.settings.BASE_URL", None
        )
        await create_snippet("ok.sh", approved=True)
        script = await snippet_source.load_script("ok.sh")
