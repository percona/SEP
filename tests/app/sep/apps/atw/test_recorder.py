...
from app.sep.apps.atw.recorder import record_atw_run, RUN_RESULT_RECORDER
...
def _recorder_uses_test_session(mocker: MockerFixture, session: AsyncSession) -> None:
    ...
    is sep-owned, not on the tasks database the recorder seam hands in. Patching the
    ...
    mocker.patch(
        "app.sep.apps.atw.recorder.get_async_session_maker", return_value=maker
    )
