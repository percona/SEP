# Copyright (C) 2026 Percona LLC
#
# This program is free software: you can redistribute it and/or modify
# it under the terms of the GNU Affero General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# This program is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
# GNU Affero General Public License for more details.
#
# You should have received a copy of the GNU Affero General Public License
# along with this program. If not, see <https://www.gnu.org/licenses/>.

"""Define tests for the inventory plugin ``inventory_schema`` object."""

from app.sep.apps.inventory.schema import inventory_schema


def test_inventory_schema_has_four_entities():
    """Assert ``inventory_schema.entities`` lists nodes, services, schemas, and tables."""
    assert inventory_schema.entities is not None
    names = [e.name for e in inventory_schema.entities]
    assert names == ["nodes", "services", "schemas", "tables"]


def test_inventory_schema_serialises_entities():
    """Assert ``inventory_schema`` serialises ``entities`` with snake_case keys for JSON."""
    dumped = inventory_schema.model_dump(mode="json", by_alias=True)
    assert dumped["name"] == "inventory"
    assert "entities" in dumped
    assert dumped["entities"][0]["display_name"] == "Nodes"
    assert "list_view" in dumped["entities"][0]
    assert dumped["entities"][3]["detail_highlights"] == {
        "create": "sql",
        "keys": "json",
    }
